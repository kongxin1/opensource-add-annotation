...
/**
 * This broker filter handles tracking the state of the broker for purposes of publishing advisory
 * messages to advisory consumers.
 */
public class AdvisoryBroker extends BrokerFilter {
	 ...
	 */
	protected final Set<VirtualDestination> virtualDestinations = Collections
			.newSetFromMap(new ConcurrentHashMap<VirtualDestination, Boolean>());
	/**
	 * This is a map to track all consumers that exist on the virtual destination so that we can
	 * fire an advisory later when they go away to remove the demand.
	 */
	protected final ConcurrentMap<ConsumerInfo, VirtualDestination> virtualDestinationConsumers = new ConcurrentHashMap<>();
	/**
	 * This is a map to track unique demand for the existence of a virtual destination so we make
	 * sure we don't send duplicate advisories.
	 */
	...
	public Subscription addConsumer(ConnectionContext context, ConsumerInfo info) throws Exception {
		...
		if (!AdvisorySupport.isAdvisoryTopic(info.getDestination())) {
			...
			// We check here whether the Destination is Temporary Destination specific or not since
			// we
			// can avoid sending advisory messages to the consumer if it only wants Temporary
			// Destination
			// notifications. If its not just temporary destination related destinations then we
			// have
			...
			if (AdvisorySupport.isVirtualDestinationConsumerAdvisoryTopic(info.getDestination())) {
				for (Iterator<ConsumerInfo> iter = virtualDestinationConsumers.keySet().iterator(); iter.hasNext();) {
					...
					ActiveMQTopic topic = AdvisorySupport.getVirtualDestinationConsumerAdvisoryTopic(key
							.getDestination());
	...
	@Override
	public Destination addDestination(ConnectionContext context, ActiveMQDestination destination, boolean create)
			throws Exception {
	...
	public void removeDestinationInfo(ConnectionContext context, DestinationInfo destInfo) throws Exception {
		...
		if (info != null) {
			...
			fireAdvisory(context, topic, info);
			ActiveMQTopic[] advisoryDestinations = AdvisorySupport.getAllDestinationAdvisoryTopics(destInfo
					.getDestination());
			for (ActiveMQTopic advisoryDestination : advisoryDestinations) {
				try {
					next.removeDestination(context, advisoryDestination, -1);
	...
	public void messageExpired(ConnectionContext context, MessageReference messageReference, Subscription subscription) {
		...
		try {
			if (!messageReference.isAdvisory()) {
				ActiveMQTopic topic = AdvisorySupport.getExpiredMessageTopic(messageReference.getMessage()
						.getDestination());
				...
				advisoryMessage.setStringProperty(AdvisorySupport.MSG_PROPERTY_MESSAGE_ID, payload.getMessageId()
						.toString());
	...
	public void messageConsumed(ConnectionContext context, MessageReference messageReference) {
		...
		try {
			if (!messageReference.isAdvisory()) {
				ActiveMQTopic topic = AdvisorySupport.getMessageConsumedAdvisoryTopic(messageReference.getMessage()
						.getDestination());
				Message payload = messageReference.getMessage().copy();
				if (!isIncludeBodyForAdvisory(messageReference.getMessage().getDestination())) {
					payload.clearBody();
				...
				if (destination != null) {
					advisoryMessage.setStringProperty(AdvisorySupport.MSG_PROPERTY_DESTINATION, payload.getMessageId()
							.toString());
				}
				fireAdvisory(context, topic, payload, null, advisoryMessage);
			}
	...
	public void messageDelivered(ConnectionContext context, MessageReference messageReference) {
		...
		try {
			if (!messageReference.isAdvisory()) {
				ActiveMQTopic topic = AdvisorySupport.getMessageDeliveredAdvisoryTopic(messageReference.getMessage()
						.getDestination());
				Message payload = messageReference.getMessage().copy();
				if (!isIncludeBodyForAdvisory(messageReference.getMessage().getDestination())) {
					payload.clearBody();
	...
	public void messageDiscarded(ConnectionContext context, Subscription sub, MessageReference messageReference) {
		...
		try {
			if (!messageReference.isAdvisory()) {
				ActiveMQTopic topic = AdvisorySupport.getMessageDiscardedAdvisoryTopic(messageReference.getMessage()
						.getDestination());
				Message payload = messageReference.getMessage().copy();
				if (!isIncludeBodyForAdvisory(messageReference.getMessage().getDestination())) {
					payload.clearBody();
				...
				advisoryMessage.setStringProperty(AdvisorySupport.MSG_PROPERTY_MESSAGE_ID, payload.getMessageId()
						.toString());
				advisoryMessage.setStringProperty(AdvisorySupport.MSG_PROPERTY_CONSUMER_ID, sub.getConsumerInfo()
	...
	public void slowConsumer(ConnectionContext context, Destination destination, Subscription subs) {
		...
		try {
			if (!AdvisorySupport.isAdvisoryTopic(destination.getActiveMQDestination())) {
				ActiveMQTopic topic = AdvisorySupport
						.getSlowConsumerAdvisoryTopic(destination.getActiveMQDestination());
				ActiveMQMessage advisoryMessage = new ActiveMQMessage();
				advisoryMessage.setStringProperty(AdvisorySupport.MSG_PROPERTY_CONSUMER_ID, subs.getConsumerInfo()
						.getConsumerId().toString());
	...
	public void fastProducer(ConnectionContext context, ProducerInfo producerInfo, ActiveMQDestination destination) {
		...
		try {
			if (!AdvisorySupport.isAdvisoryTopic(destination)) {
				...
				ActiveMQMessage advisoryMessage = new ActiveMQMessage();
				advisoryMessage.setStringProperty(AdvisorySupport.MSG_PROPERTY_PRODUCER_ID, producerInfo
						.getProducerId().toString());
						...
						if (getBrokerService().isUseVirtualDestSubsOnCreation()) {
							...
							for (ActiveMQDestination destination : destinations.keySet()) {
								if (destination.isQueue()) {
									if (virtualDestinationMatcher.matches(virtualDestination, destination)) {
										fireVirtualDestinationAddAdvisory(context, null, destination,
												virtualDestination);
									}
								}
							}
						}
						// loop through existing consumers to see if any of them are consuming on a
						// destination
	...
	private void fireVirtualDestinationAddAdvisory(ConnectionContext context, ConsumerInfo info,
			ActiveMQDestination activeMQDest, VirtualDestination virtualDestination) throws Exception {
		...
		if (info == null) {
			...
			// store the virtual destination and the activeMQDestination as a pair so that we can
			// keep track
					...
					if (getBrokerService().isUseVirtualDestSubsOnCreation()) {
						if (!AdvisorySupport.isAdvisoryTopic(virtualDestination.getVirtualDestination())) {
							for (ConsumerInfo info : virtualDestinationConsumers.keySet()) {
								...
								// check consumers created for the existence of a destination to see
								// if they
	...
	private void fireVirtualDestinationRemoveAdvisory(ConnectionContext context, ConsumerInfo info) throws Exception {
		...
		if (virtualDestination != null) {
			ActiveMQTopic topic = AdvisorySupport.getVirtualDestinationConsumerAdvisoryTopic(virtualDestination
	...
	@Override
	/**
	 * 构建一个名字为“ActiveMQ.Advisory.MasterBroker”的topic，并构建对应的消息体，将该消息send出去
	 * @Title: nowMasterBroker 
	 * @Description: TODO
	 * @return: void
	 */
	...
	public boolean sendToDeadLetterQueue(ConnectionContext context, MessageReference messageReference,
		...
		if (wasDLQd) {
			try {
				if (!messageReference.isAdvisory()) {
					ActiveMQTopic topic = AdvisorySupport.getMessageDLQdAdvisoryTopic(messageReference.getMessage()
							.getDestination());
					Message payload = messageReference.getMessage().copy();
					if (!isIncludeBodyForAdvisory(messageReference.getMessage().getDestination())) {
						payload.clearBody();
	...
	protected boolean isIncludeBodyForAdvisory(ActiveMQDestination activemqDestination) {
		...
		return (destination instanceof BaseDestination && ((BaseDestination) destination).isIncludeBodyForAdvisory()) ? true
				: false;
	}
	...
	protected void fireConsumerAdvisory(ConnectionContext context, ActiveMQDestination consumerDestination,
			ActiveMQTopic topic, Command command) throws Exception {
		fireConsumerAdvisory(context, consumerDestination, topic, command, null);
	}
	protected void fireConsumerAdvisory(ConnectionContext context, ActiveMQDestination consumerDestination,
			ActiveMQTopic topic, Command command, ConsumerId targetConsumerId) throws Exception {
		ActiveMQMessage advisoryMessage = new ActiveMQMessage();
		int count = 0;
	...
	protected void fireProducerAdvisory(ConnectionContext context, ActiveMQDestination producerDestination,
			ActiveMQTopic topic, Command command) throws Exception {
	...
	/**
	 * 设置消息对象ActiveMQMessage和生产者状态信息ProducerBrokerExchange，并调用send方法发送消息
	 * @Title: fireAdvisory
	 * @Description: TODO
	 * @param context
	 * @param topic
	 * @param command
	 * @param targetConsumerId
	 * @param advisoryMessage
	 * @throws Exception
	 * @return: void
	 */
	public void fireAdvisory(ConnectionContext context, ActiveMQTopic topic, Command command,
			ConsumerId targetConsumerId, ActiveMQMessage advisoryMessage) throws Exception {
		if (getBrokerService().isStarted()) {
			// set
			// properties 构建一个ActiveMQMessage对象
	...
	private class VirtualConsumerPair {
		...
		public int hashCode() {
			...
			result = prime * result + ((activeMQDestination == null) ? 0 : activeMQDestination.hashCode());
			result = prime * result + ((virtualDestination == null) ? 0 : virtualDestination.hashCode());
			return result;
		}
		@Override
