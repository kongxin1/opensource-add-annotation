 ...
 * <P>
 * A client also has the option of creating a message producer without supplying a destination. In
 * this case, a destination must be provided with every send operation. A typical use for this kind
 * of message producer is to send replies to requests using the request's <CODE>JMSReplyTo</CODE>
 * destination.
 * <P>
 * A client can specify a default delivery mode, priority, and time to live for messages sent by a
 * message producer. It can also specify the delivery mode, priority, and time to live for an
 * individual message.
 * <P>
 * A client can specify a time-to-live value in milliseconds for each message it sends. This value
 * defines a message expiration time that is the sum of the message's time-to-live and the GMT when
 * it is sent (for transacted sends, this is the time the client sends the message, not the time the
 * transaction is committed).
 * <P>
 * A JMS provider should do its best to expire messages accurately; however, the JMS API does not
 * define the accuracy provided.
...
public class ActiveMQMessageProducer extends ActiveMQMessageProducerSupport implements StatsCapable, Disposable {
	...
	protected ActiveMQMessageProducer(ActiveMQSession session, ProducerId producerId, ActiveMQDestination destination,
		...
		if (destination != null && destination.getOptions() != null) {
			Map<String, Object> options = IntrospectionSupport.extractProperties(new HashMap<String, Object>(
					destination.getOptions()), "producer.");
			IntrospectionSupport.setProperties(this.info, options);
			if (options.size() > 0) {
				String msg = "There are " + options.size() + " producer options that couldn't be set on the producer."
						+ " Check the options are spelled correctly." + " Unknown parameters=[" + options + "]."
	 ...
	 * MessageProducer</CODE> outside the Java virtual machine, clients should close them when they
	 * are not needed. Relying on garbage collection to eventually reclaim these resources may not
	 * be timely enough.
	 * @throws JMSException if the JMS provider fails to close the producer due to some internal
	 *             error.
	 */
	 ...
	 * <P>
	 * Typically, a message producer is assigned a destination at creation time; however, the JMS
	 ...
	 * @throws JMSException if the JMS provider fails to send the message due to some internal
	 *             error.
	...
	public void send(Message message, AsyncCallback onComplete) throws JMSException {
		this.send(this.getDestination(), message, this.defaultDeliveryMode, this.defaultPriority,
	...
	public void send(Message message, int deliveryMode, int priority, long timeToLive, AsyncCallback onComplete)
			throws JMSException {
	...
	public void send(Destination destination, Message message, int deliveryMode, int priority, long timeToLive,
		...
		if (destination.equals(info.getDestination())) {// 判断两个地址是否一致
