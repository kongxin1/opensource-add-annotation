 ...
 */
/**
 * 一个对象代表一个消息，里面大部分方法都是get和set方法
 * @ClassName: Message
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月16日 下午1:48:39
 */
public abstract class Message extends BaseCommand implements MarshallAware, MessageReference {
	...
	// 标示本对象中的属性是否是只读的，如果是只读的，不能调用setObjectProperty方法对属性做修改
	...
	private Map<String, Object> unmarsallProperties(ByteSequence marshalledProperties) throws IOException {
		return MarshallingSupport.unmarshalPrimitiveMap(new DataInputStream(new ByteArrayInputStream(
	...
	/**
	 * Can be used to route the message to a specific consumer. Should be null to allow the broker
	 * use normal JMS routing semantics. If the target consumer id is an active consumer on the
	 * broker, the message is dropped. Used by the AdvisoryBroker to replay advisory messages to a
	 * specific consumer.
	 * @openwire:property version=1 cache=true
	 */
	@Override
	...
	/**
	 * Only set by the broker and defines the userID of the producer connection who sent this
	 * message. This is an optional field, it needs to be enabled on the broker to have this field
	 * populated.
	 * @openwire:property version=1
	 */
	...
	/**
	 * 对消息体进行压缩，直接调用DeflaterOutputStream类进行压缩，压缩算法应该是Deflater
	 * @Title: doCompress
	 * @Description: TODO
	 * @throws IOException
	 * @return: void
