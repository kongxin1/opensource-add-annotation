...
public abstract class ActiveMQDestination extends JNDIBaseStorable implements DataStructure, Destination,
		Externalizable, Comparable<Object> {
	...
	// 如果physicalName中含有* 或者>，则isPattern=true
	 ...
	 * @Description: 根据名字的前缀判断需要创建队列还是topic，如果没有前缀，就根据第二个入参创建对应的queue或者topic
	 *               可以创建的队列有四种：queue，topic，temp-topic,temp-queue
	 * @param name
	 * @param defaultType
	 * @return
	...
	public static ActiveMQDestination createDestination(String name, byte defaultType) {
		...
		// 如果名字都不符合要求，就按照defaultType创建对应的队列
		switch (defaultType) {
			case QUEUE_TYPE:
	...
	/**
	 * 设置复合destination对象
	 * @Title: setCompositeDestinations
	 * @Description: TODO
	 * @param destinations
	 * @return: void
	 */
	...
	public void setPhysicalName(String physicalName) {
		...
		this.destinationPaths = null;
		this.hashValue = 0;
		// 如果是一个复合的名字，就将名字拆开，针对每一个名字创建一个ActiveMQDestination对象
		if (composite) {
	...
	/**
	 * 如果destinationPaths为null，就将physicalName按“.”为分隔符进行分割，将分割后的字符串作为元素放入destinationPaths中
	 * @Title: getDestinationPaths
	 * @Description: TODO
	 * @return
	 * @return: String[]
	 */
