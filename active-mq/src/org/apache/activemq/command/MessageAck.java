...
/**
 * 消息的确认对象
 * @ClassName: MessageAck
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年8月12日 下午7:31:12
 */
public class MessageAck extends BaseCommand {
	public static final byte DATA_STRUCTURE_TYPE = CommandTypes.MESSAGE_ACK;
	/**
	 * Used to let the broker know that the message has been delivered to the client. Message will
	 * still be retained until an standard ack is received. This is used get the broker to send more
	 * messages past prefetch limits when an standard ack has not been sent.
	 */
	...
	/**
	 * In case the client want's to explicitly let the broker know that a message was not processed
	 * and the message was considered a poison message.
	 */
	public static final byte POSION_ACK_TYPE = 1;
	/**
	 * In case the client want's to explicitly let the broker know that a message was not processed
	 * and it was re-delivered to the consumer but it was not yet considered to be a poison message.
	 * The messageCount field will hold the number of times the message was re-delivered.
	 */
