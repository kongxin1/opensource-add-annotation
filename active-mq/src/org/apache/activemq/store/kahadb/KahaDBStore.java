...
public class KahaDBStore extends MessageDatabase implements PersistenceAdapter {
	...
	public static final int cancelledTaskModMetric = Integer.parseInt(
			System.getProperty(PROPERTY_CANCELED_TASK_MOD_METRIC, "0"), 10);
	...
	private static final int asyncExecutorMaxThreads = Integer.parseInt(
			System.getProperty(PROPERTY_ASYNC_EXECUTOR_MAX_THREADS, "1"), 10);;
		...
		if (brokerService != null) {
			...
			if (metadata.openwireVersion != brokerService.getStoreOpenWireVersion()) {
				LOG.warn("Existing Store uses a different OpenWire version[{}] "
						...
						+ "used by this store, some newer broker features may not work" + "as expected.",
						metadata.openwireVersion, brokerService.getStoreOpenWireVersion());
	...
	public class KahaDBMessageStore extends AbstractMessageStore {
		...
		/**
		 * 其实这个对象也就记住了一个destination对象
		 * @Title:KahaDBMessageStore
		 * @Description:TODO
		 * @param destination
		 */
		public KahaDBMessageStore(ActiveMQDestination destination) {
			super(destination);
			// 将入参转化为KahaDestination对象
		...
		public void addMessage(final ConnectionContext context, final Message message) throws IOException {
			...
			command.setTransactionInfo(TransactionIdConversion.convert(transactionIdTransformer.transform(message
					.getTransactionId())));
		...
		public void updateMessage(Message message) throws IOException {
			if (LOG.isTraceEnabled()) {
				LOG.trace("updating: " + message.getMessageId() + " with deliveryCount: "
		...
		public void removeMessage(ConnectionContext context, MessageAck ack) throws IOException {
			...
			command.setTransactionInfo(TransactionIdConversion.convert(transactionIdTransformer.transform(ack
					.getTransactionId())));
			...
			try {
				pageFile.tx().execute(new Transaction.Closure<Exception>() {
					...
					public void execute(Transaction tx) throws Exception {
						...
						for (Iterator<Entry<Long, MessageKeys>> iterator = sd.orderIndex.iterator(tx); listener
								.hasSpace() && iterator.hasNext();) {
			...
			for (Iterator<String> iterator = rolledBackAcks.iterator(); iterator.hasNext();) {
				...
				if (sequence != null) {
					if (sd.orderIndex.alreadyDispatched(sequence)) {
						...
						LOG.info("rolledback ack message {} with seq {} will be picked up in future batch {}", id,
								sequence, sd.orderIndex.cursor);
					}
				} else {
		...
		protected void lockAsyncJobQueue() {
			try {
				if (!this.localDestinationSemaphore.tryAcquire(this.maxAsyncJobs, 60, TimeUnit.SECONDS)) {
					throw new TimeoutException(this + " timeout waiting for localDestSem:"
							+ this.localDestinationSemaphore);
				}
			} catch (Exception e) {
		...
		@Override
		public String toString() {
			return "permits:" + this.localDestinationSemaphore.availablePermits() + ",sd="
					+ storedDestinations.get(key(dest));
		}
		@Override
		protected void recoverMessageStoreStatistics() throws IOException {
			try {
				...
				indexLock.writeLock().lock();
				try {
					recoveredStatistics = pageFile.tx().execute(
							new Transaction.CallableClosure<MessageStoreStatistics, IOException>() {
								@Override
								public MessageStoreStatistics execute(Transaction tx) throws IOException {
									...
									// Iterate through all index entries to get the size of each
									// message
									StoredDestination sd = getStoredDestination(dest, tx);
					...
					getMessageStoreStatistics().getMessageCount().setCount(
							recoveredStatistics.getMessageCount().getCount());
					getMessageStoreStatistics().getMessageSize().setTotalSize(
							recoveredStatistics.getMessageSize().getTotalSize());
				} finally {
					indexLock.writeLock().unlock();
				}
			} finally {
	...
	class KahaDBTopicMessageStore extends KahaDBMessageStore implements TopicMessageStore {
		...
		protected void doAcknowledge(ConnectionContext context, String subscriptionKey, MessageId messageId,
			...
			command.setTransactionInfo(ack != null ? TransactionIdConversion.convert(transactionIdTransformer
					.transform(ack.getTransactionId())) : null);
		...
		public void addSubscription(SubscriptionInfo subscriptionInfo, boolean retroactive) throws IOException {
			String subscriptionKey = subscriptionKey(subscriptionInfo.getClientId(),
	...
	public MessageStore createQueueMessageStore(ActiveMQQueue destination) throws IOException {
		...
		if (store == null) {
			...
			// 将得到的MessageStore对象添加到缓存中，如果此时缓存中已经包含了key对应的值，那么就不在往缓存中放了
	...
	@Override
	/**
	 * 得到topic的消息存储器，存储器会被缓存到一个map属性中
	 */
	public TopicMessageStore createTopicMessageStore(ActiveMQTopic destination) throws IOException {
		// 得到入参对应的key，convert方法内部转化操作得到的对象仅仅是为了得到key，其他作用没有
		String key = key(convert(destination));
		MessageStore store = storeCache.get(key(convert(destination)));
		if (store == null) {
	...
	/**
	 * Cleanup method to remove any state associated with the given destination. This method does
	 * not stop the message store (it might not be cached).
	...
	/**
	 * Cleanup method to remove any state associated with the given destination This method does not
	 * stop the message store (it might not be cached).
	 * @param destination Destination to forget
	 */
	@Override
	...
	/**
	 * 将入参转化为KahaDestination对象，这个对象会被设置两个属性：队列名字和队列类型
	 * @Title: convert
	 * @Description: TODO
	 * @param dest
	 * @return
	...
	public class StoreTaskExecutor extends ThreadPoolExecutor {
		public StoreTaskExecutor(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit timeUnit,
