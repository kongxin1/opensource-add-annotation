 ...
 */
/**
 * 本对象就是一个B数节点，从属性中就可以看出来
 * @ClassName: BTreeNode
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年8月14日 下午8:20:29
 * @param <Key>
 * @param <Value>
 */
public final class BTreeNode<Key, Value> {
	...
	/**
	 * 一个对象代表的是B树中一个节点中的一个元素，就是节点中指针指的的key和value
	 * @ClassName: KeyValueEntry
	 * @Description: TODO
	 * @author: 孔新
	 * @date: 2016年8月15日 下午9:22:52
	 */
	...
	/**
	 * B树迭代器，可以对B树进行遍历
	 * @ClassName: BTreeIterator
	 * @Description: TODO
	 * @author: 孔新
	 * @date: 2016年8月15日 下午9:16:32
	 */
	...
	/**
	 * Internal (to the BTreeNode) method. Because this method is called only by BTreeNode itself,
	...
	public Value remove(Transaction tx, Key key) throws IOException {
		if (isBranch()) {
			...
			if (child.keys.length == 0) {
				...
				if (child.isBranch()) {
					// This is cause branches are never really empty.. they just go down to 1
					// child..
					children[idx] = child.children[0];
					tx.free(child.getPage());
				} else {
					// The child was a leaf. Then we need to actually remove it from this branch
					// node..
		...
		if (isBranch()) {
			for (int i = 0; i < children.length; i++) {
				...
				if (i == children.length - 1) {
					out.println(prefix + "\\- " + child.getPageId()
							+ (child.isBranch() ? " (" + child.children.length + ")" : ""));
					child.printStructure(tx, out, prefix + "   ");
				} else {
					out.println(prefix + "|- " + child.getPageId()
							+ (child.isBranch() ? " (" + child.children.length + ")" : "") + " : " + keys[i]);
					child.printStructure(tx, out, prefix + "   ");
				}
			}
		}
	 ...
	 * @return
	 * @throws IOException
	 * @return: Iterator<Map.Entry<Key,Value>>
	 */
	...
	/**
	 * 找到入参node所应该在B树中位置，从这个搜索可以看出B树其实是B+树
	 * @Title: getLeafNode
	 * @Description: TODO
	 * @param tx
	 * @param node
	 ...
	 */
	private static <Key, Value> BTreeNode<Key, Value> getLeafNode(Transaction tx, final BTreeNode<Key, Value> node,
	...
	// /////////////////////////////////////////////////////////////////
	/**
	 * 是否允许超出限额，如果节点中的key个数<=3，则可以，否则该页需要分裂，从这里可以看出一个节点最多有3个key
	 * @Title: allowOverflow
	 * @Description: TODO
	 * @return
	 * @return: boolean
	...
	private boolean allowOverflow() {
		// Only allow page overflow if there are <= 3 keys in the node. Otherwise a split will occur
		// on overflow
	...
	@SuppressWarnings("unchecked")
	/**
	 * 将val插入到idx所指示的位置，val插入位置的下标=idx
	 * @Title: arrayInsert 
	 * @Description: TODO
	 * @param vals
	 * @param val
