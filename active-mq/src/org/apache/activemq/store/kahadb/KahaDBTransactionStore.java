...
/**
 * Provides a TransactionStore implementation that can create transaction aware MessageStore objects
 * from non transaction aware MessageStore objects.
 */
public class KahaDBTransactionStore implements TransactionStore {
	...
	/**
	 * 添加消息事务命令对象
	 * @ClassName: AddMessageCommand
	 * @Description: TODO
	 * @author: 孔新
	 * @date: 2016年8月15日 下午9:02:48
	 */
	...
	/**
	 * 删除消息事务命令对象
	 * @ClassName: RemoveMessageCommand
	 * @Description: TODO
	 * @author: 孔新
	 * @date: 2016年8月15日 下午9:03:07
	 */
		...
		if (txid != null) {
			if (!txid.isXATransaction() && theStore.isConcurrentStoreAndDispatchTransactions()) {
				...
				if (tx != null) {
					...
					if (doneSomething) {
						KahaTransactionInfo info = getTransactionInfo(txid);
						theStore.store(new KahaCommitCommand().setTransactionInfo(info),
				...
				theStore.store(new KahaCommitCommand().setTransactionInfo(info), theStore.isEnableJournalDiskSyncs(),
						null, postCommit);
	...
	public void rollback(TransactionId txid) throws IOException {
		if (txid.isXATransaction() || theStore.isConcurrentStoreAndDispatchTransactions() == false) {
			KahaTransactionInfo info = getTransactionInfo(txid);
			theStore.store(new KahaRollbackCommand().setTransactionInfo(info), theStore.isEnableJournalDiskSyncs(),
					null, null);
		...
		for (Map.Entry<TransactionId, List<Operation>> entry : theStore.preparedTransactions.entrySet()) {
			...
			for (Operation op : entry.getValue()) {
				if (op.getClass() == MessageDatabase.AddOperation.class) {
					...
					Message msg = (Message) wireFormat().unmarshal(
							new DataInputStream(addOp.getCommand().getMessage().newInput()));
					messageList.add(msg);
				} else {
	...
	ListenableFuture<Object> asyncAddQueueMessage(ConnectionContext context, final MessageStore destination,
	...
	ListenableFuture<Object> asyncAddTopicMessage(ConnectionContext context, final MessageStore destination,
			final Message message) throws IOException {
		if (message.getTransactionId() != null) {
			if (message.getTransactionId().isXATransaction()
					|| theStore.isConcurrentStoreAndDispatchTransactions() == false) {
	...
	final void acknowledge(ConnectionContext context, final TopicMessageStore destination, final String clientId,
			final String subscriptionName, final MessageId messageId, final MessageAck ack) throws IOException {
		if (ack.isInTransaction()) {
			if (ack.getTransactionId().isXATransaction()
					|| theStore.isConcurrentStoreAndDispatchTransactions() == false) {
