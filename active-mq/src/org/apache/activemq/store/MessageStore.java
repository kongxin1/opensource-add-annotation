 ...
 */
/**
 * 消息存储器，消息存储在本对象中，Topic的消息存储器是本接口的子接口
 * @ClassName: MessageStore
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月5日 下午10:49:19
 */
public interface MessageStore extends Service {
	 ...
	 * @param context context
	 * @param message
	 * @param canOptimizeHint - give a hint to the store that the message may be consumed before it
	 *            hits the disk
	 * @throws IOException
	 */
	 ...
	 * @throws IOException
	 */
	ListenableFuture<Object> asyncAddQueueMessage(ConnectionContext context, Message message, boolean canOptimizeHint)
			throws IOException;
	/**
	 * Adds a message to the message store
	 * @param context context
	 ...
	 * @throws IOException
	 */
	ListenableFuture<Object> asyncAddTopicMessage(ConnectionContext context, Message message, boolean canOptimizeHint)
			throws IOException;
	/**
	 * Looks up a message using either the String messageID or the messageNumber. Implementations
	 * are encouraged to fill in the missing key if its easy to do so.
	 ...
	 * @param context
	 * @param ack the ack request that cause the message to be removed. It conatins the identity
	 *            which contains the messageID of the message that needs to be removed.
	 * @throws IOException
	 */
	...
	/**
	 * @param memoryUsage The SystemUsage that is controlling the destination's memory usage.
	 */
