...
/**
 * 持久化适配器接口
 * @ClassName: PersistenceAdapter
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月5日 下午10:39:18
 */
public interface PersistenceAdapter extends Service {
	/**
	 * Returns a set of all the {@link org.apache.activemq.command.ActiveMQDestination} objects that
	 ...
	 * @throws UnsupportedOperationException If this adapter does not provide its own scheduler
	...
	/**
	 * This method starts a transaction on the persistent storage - which is nothing to do with JMS
	 * or XA transactions - its purely a mechanism to perform multiple writes to a persistent store
	 * in 1 transaction as a performance optimization.
	 * <p/>
	 * Typically one transaction will require one disk synchronization point and so for real high
	 * performance its usually faster to perform many writes within the same transaction to minimize
	 * latency caused by disk synchronization. This is especially true when using tools like
	 * Berkeley Db or embedded JDBC servers.
	 * @param context
	 * @throws IOException
	 */
	...
	long size();
	/**
	 * return the last stored producer sequenceId for this producer Id used to suppress duplicate
	 * sends on failover reconnect at the transport when a reconnect occurs
