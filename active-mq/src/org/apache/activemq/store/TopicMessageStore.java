 ...
 */
/**
 * Topic的消息存储器，Topic的消息存储在本对象中
 * @ClassName: TopicMessageStore
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月5日 下午10:50:10
 */
public interface TopicMessageStore extends MessageStore {
	/**
	 * Stores the last acknowledged messgeID for the given subscription so that we can recover and
	 * commence dispatching messages from the last checkpoint
	 * @param context
	 * @param clientId
	...
	/**
	 * For the new subscription find the last acknowledged message ID and then find any new messages
	 * since then and dispatch them to the subscription.
	 * <p/>
	...
	void recoverSubscription(String clientId, String subscriptionName, MessageRecoveryListener listener)
			throws Exception;
	/**
	 * For an active subscription - retrieve messages from the store for the subscriber after the
	 ...
	 * <p/>
	 * If this is a new subscription and the retroactive is false, then the last message sent to the
	 * topic should be set as the last message acknowledged by they new subscription. Otherwise, if
	 * retroactive is true, then create the subscription without it having an acknowledged message
	 * so that on recovery, all message recorded for the topic get replayed.
