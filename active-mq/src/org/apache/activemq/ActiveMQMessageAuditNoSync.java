...
public class ActiveMQMessageAuditNoSync implements Serializable {
	...
	public void setMaximumNumberOfProducersToTrack(int maximumNumberOfProducersToTrack) {
		if (maximumNumberOfProducersToTrack < this.maximumNumberOfProducersToTrack) {
			LRUCache<String, BitArrayBin> newMap = new LRUCache<String, BitArrayBin>(0,
					maximumNumberOfProducersToTrack, 0.75f, true);
			/**
			 * As putAll will access the entries in the right order, this shouldn't result in wrong
			 * cache entries being removed
			 */
	...
	/**
	 * Returns if the Audit has been modified since last check, this method does not reset the
	 * modified flag. If the caller needs to reset the flag in order to avoid serializing an
	 * unchanged Audit then its up the them to reset it themselves.
	...
	/**
	 * Reads and returns the current modified state of the Audit, once called the state is reset to
	 * false. This method is useful for code the needs to know if it should write out the Audit or
	 * otherwise execute some logic based on the Audit having changed since last check.
