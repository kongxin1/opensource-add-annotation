...
/**
 * Utility class that provides methods for parsing URI's This class can be used to split composite
 * URI's into their component parts and is used to extract any URI options from each URI in order to
 * set specific properties on Beans.
 */
public class URISupport {
	 ...
	 */
	/**
	 * @ClassName: CompositeData
	 * @Description: 该类对应了一个URI对应，相当于对URI进行了完全解析拆分放到该对象的不同属性上
	 * @author: 孔新
	 * @date: 2016年7月3日 上午12:53:27
	 */
	public static class CompositeData {
		...
		/**
		 * @Title: toURI
		 * @Description: 将本对象中的各种属性值组装在一起形成一个URI对象
		 * @return
	...
	/**
	 * @Title: parseQuery
	 * @Description: 分析URI中？之后的内容，将？之后的内容解析后放入一个Map对象中，并返回
	 * @param uri
	 * @return
	 * @throws URISyntaxException
	...
	public static Map<String, String> parseQuery(String uri) throws URISyntaxException {
		try {
			uri = uri.substring(uri.lastIndexOf("?") + 1); // get only the relevant part of the
															// query
	 ...
	 * This method differs from the {@link parseQuery} method in that it handles composite URI types
	...
	public static Map<String, String> parseParameters(URI uri) throws URISyntaxException {
		// 不是组合uri，即uri去除schema和“\\”之后不是以“（”开始的
	 ...
	 * Given a Key / Value mapping create and append a URI query value that represents the mapped
	 * entries, return the newly updated URI that contains the value of the given URI and the
	 * appended query value. Each entry in the query string is prefixed by the supplied optionPrefix
	 * string.
	 * @param uri The source URI that will have the Map entries appended as a URI query value.
	 * @param queryParameters The Key / Value mapping that will be transformed into a URI query
	 *            string.
	 ...
	 */
	/**
	 * 将原来uri中的参数删除，添加上query的参数
	 * @Title: createURIWithQuery
	 * @Description: TODO
	 * @param uri
	 * @param query
	 * @return
	 ...
	 */
	/**
	 * 将uri拆分成CompositeData对象中的各个属性
	 * @Title: parseComposite
	 * @Description: TODO
	 * @param uri
	 * @return
	 * @throws URISyntaxException
	...
	public static CompositeData parseComposite(URI uri) throws URISyntaxException {
		...
		// getRawSchemeSpecificPart方法根据uri的字符串按“:”进行分割，返回的是第一个冒号后面的所有内容，如果没有冒号，就原样返回
	 ...
	 * @param first The index in the string of the opening parend whose close value is to be
	 *            searched.
	 ...
	 */
	/**
	 * 找到个数完全匹配的左右括号后，第一个字符在str中的索引值
	 * @Title: indexOfParenthesisMatch
	 * @Description: TODO
	 * @param str
	 * @param first
	 * @return
	 ...
	 * from the source URI, parse the composite URI and populate the CompositeData object with the
	 * results. The source URI is used only for logging as the ssp should have already been
	 ...
	 * @param ssp The scheme specific part from the original string that is a composite or one or
	 *            more URIs.
	 * @throws URISyntaxException
	 */
	/**
	 * 处理组合uri，组合uri就是多个uri使用同一个schema，然后多个uri使用括号包起来
	 * @Title: parseComposite
	 * @Description: TODO
	 * @param uri
	 * @param rc
	...
	private static void parseComposite(URI uri, CompositeData rc, String ssp) throws URISyntaxException {
		...
		if (initialParen == 0) {
			...
			// componentString中存储的是所有uri，此时的uri还在（）中包着，没有解析
	 ...
	 */
	/**
	 * 将str进行拆分，以（）为单位，一个括号单位放在一起，括号单位之间使用“，”分隔，每个括号单位都放在数组中，最后返回数组，括号单位中不一定只有一对括号
	 * @Title: splitComponents
	 * @Description: TODO
	 * @param str
	 * @return
	 * @return: String[]
	 ...
	 */
	/**
	 * 如果value包含prefix前缀，那么就将value中的前缀去除
	 * @Title: stripPrefix
	 * @Description: TODO
	 * @param value
	 * @param prefix
	 ...
	 */
	/**
	 * 将options中的元素构建成一个字符串，就是url问号后面那种的字符串
	...
	/**
	 * Creates a URI from the original URI and the remaining parameters. When the query options of a
	 * URI are applied to certain objects the used portion of the query options needs to be removed
	 * and replaced with those that remain so that other parts of the code can attempt to apply the
	 * remainder or give an error is unknown values were given. This method is used to update a URI
	 ...
	 */
	/**
	 * 将params中的内容作为参数设置到originalURI上
	 * @Title: createRemainingURI
	 * @Description: TODO
	 * @param originalURI
	 * @param params
	 * @return
	 ...
	 */
	/**
	 * 判断str入参中的左右括号的个数是否匹配，如果不相等返回false，相同返回true
	 * @Title: checkParenthesis
	 * @Description: TODO
	 * @param str
	 * @return
	 * @return: boolean
