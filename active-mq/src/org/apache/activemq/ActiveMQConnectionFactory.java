 ...
 * <p/>
 * This class also implements QueueConnectionFactory and TopicConnectionFactory. You can use this
 * connection to create both QueueConnections and TopicConnections.
 * @see javax.jms.ConnectionFactory
 */
public class ActiveMQConnectionFactory extends JNDIBaseStorable implements ConnectionFactory, QueueConnectionFactory,
		TopicConnectionFactory, StatsCapable, Cloneable {
		...
		try {
			host = AccessController.doPrivileged(new PrivilegedAction<String>() {
				...
				public String run() {
					...
					result = (result == null || result.isEmpty()) ? System.getProperty("AMQ_HOST", "localhost")
							: result;
					return result;
				}
			});
		...
		try {
			bindURL = AccessController.doPrivileged(new PrivilegedAction<String>() {
				...
				public String run() {
					...
					result = (result == null || result.isEmpty()) ? System.getProperty("BROKER_BIND_URL", defaultURL)
							: result;
					return result;
				}
			});
	...
	private static URI createURI(String brokerURL) {
		try {
			...
			throw (IllegalArgumentException) new IllegalArgumentException("Invalid broker URI: " + brokerURL)
					.initCause(e);
		}
	}
	/**
	...
	/**
	 * Creates a Transport based on this object's connection settings. Separated from
	 * createActiveMQConnection to allow for subclasses to override.
	 * @return The newly created Transport.
	 * @throws JMSException If unable to create trasnport.
	 */
	/**
	 * 根据brokerURL中的schema值的不同，创建Transport对象
	 ...
	 */
	/**
	 * 创建socket对象，并与服务器建立连接
	 * @Title: createActiveMQConnection
	 * @Description: TODO
	 * @param userName
	 * @param password
	 * @return
	...
	protected ActiveMQConnection createActiveMQConnection(String userName, String password) throws JMSException {
		...
		try {
			Transport transport = createTransport();// 创建出transport对象，里面含有一个空的socket对象
			// 创建出ActiveMQConnection对象，这个对象目前只是设置一部分属性
	...
	/**
	 * 创建出ActiveMQConnection对象，这个对象里面只是进行了属性的设置
	 * @Title: createActiveMQConnection
	 * @Description: TODO
	 * @param transport
	 * @param stats
	...
	/**
	 * 设置ActiveMQConnection属性
	 * @Title: configureConnection
	 * @Description: TODO
	 * @param connection
	 * @throws JMSException
	 * @return: void
	...
	/**
	 * Sets the <a href="http://activemq.apache.org/configuring-transports.html">connection URL</a>
	 * used to connect to the ActiveMQ broker.
	 */
	/**
	 * 对url进行分析，设置一些策略对象，如果url中有参数的话，就将参数中的内容设置到本对象中，如果没有就将分析结束后的url设置到brokerURL属性中
	 * @Title: setBrokerURL
	 * @Description: TODO
	 * @param brokerURL
	 * @return: void
	...
	public void setBrokerURL(String brokerURL) {
		...
		if (this.brokerURL.getQuery() != null) {
			...
			try {
				...
				// 设置消息预取策略、消息重发策略和blob的发送策略，并将url的参数中的部分内容设置到本对象中，如果都能正常设置，就返回true
				if (buildFromMap(jmsOptionsMap)) {
					if (!jmsOptionsMap.isEmpty()) {
						String msg = "There are " + jmsOptionsMap.size()
								...
								+ " Check the options are spelled correctly." + " Unknown parameters=[" + jmsOptionsMap
			...
			try {
				...
				Map<String, Object> jmsOptionsMap = IntrospectionSupport
						.extractProperties(data.getParameters(), "jms.");
				if (buildFromMap(jmsOptionsMap)) {
					if (!jmsOptionsMap.isEmpty()) {
						String msg = "There are " + jmsOptionsMap.size()
								+ " jms options that couldn't be set on the ConnectionFactory."
	...
	/**
	 * Sets the JMS clientID to use for the created connection. Note that this can only be used by
	 * one connection at once so generally its a better idea to set the clientID on a Connection
	 */
	 ...
	 * it if you do not mutate JMS messages after they are sent for a performance boost
	 */
	...
	/**
	 * Sets whether or not timestamps on messages should be disabled or not. If you disable them it
	 * adds a small performance boost.
	 */
	...
	/**
	 * If this flag is set then an larger prefetch limit is used - only applicable for durable topic
	 * subscribers.
	 */
	...
	/**
	 * Sets the <a href="http://activemq.apache.org/what-is-the-prefetch-limit-for.html">prefetch
	...
	/**
	 * Forces the use of <a href="http://activemq.apache.org/async-sends.html">Async Sends</a> which
	 * adds a massive performance boost; but means that the send() method will return immediately
	 * whether the message has been sent or not which could lead to message loss.
	 */
	...
	/**
	 * Enables or disables whether or not queue consumers should be exclusive or not for example to
	 * preserve ordering when not using <a
	...
	/**
	 * Sets the global default redelivery policy to be used when a message is delivered but the
	...
	/**
	 * Sets the global redelivery policy mapping to be used when a message is delivered but the
	 * session is rolled back
	 */
	...
	/**
	 * 设置消息预取策略、消息重发策略和blob的发送策略，并将url的参数中的部分内容设置到本对象中
	 * @Title: buildFromMap
	 * @Description: TODO
	 * @param properties
	 * @return
	...
	public void populateProperties(Properties props) {
		...
		props.setProperty("consumerFailoverRedeliveryWaitPeriod",
				Long.toString(getConsumerFailoverRedeliveryWaitPeriod()));
	...
	/**
	 * When an object is set on an ObjectMessage, the JMS spec requires the object to be serialized
	 * by that set method. Enabling this flag causes the object to not get serialized. The object
	 * may subsequently get serialized if the message needs to be sent over a socket or stored to
	 * disk.
	 */
	...
	/**
	 * Enables or disables the default setting of whether or not consumers have their messages <a
	 * href="http://activemq.apache.org/consumer-dispatch-async.html">dispatched synchronously or
	 * asynchronously by the broker</a>. For non-durable topics for example we typically dispatch
	 * synchronously by default to minimize context switches which boost performance. However
	 * sometimes its better to go slower to ensure that a single blocked consumer socket does not
	 * block delivery to other consumers.
	 * @param asyncDispatch If true then consumers created on this connection will default to having
	 *            their messages dispatched asynchronously. The default value is true.
	 */
	...
	/**
	 * Sets the timeout before a close is considered complete. Normally a close() on a connection
	 * waits for confirmation from the broker; this allows that operation to timeout to save the
	 * client hanging if there is no broker
	 */
	...
	/**
	 * If this flag is not set then a separate thread is not used for dispatching messages for each
	 * Session in the Connection. However, a separate thread is always used if there is more than
	 * one session, or the session isn't in auto acknowledge or duplicates ok mode. By default this
	 * value is set to true and session dispatch happens asynchronously.
	 */
	...
	/**
	 * Enables/disables whether or not Message properties and MapMessage entries support <a
	...
	/**
	 * Sets the prefix used by autogenerated JMS Client ID values which are used if the JMS client
	 * does not explicitly specify on.
	...
	/**
	 * Enables the timeout from a connection creation to when a warning is generated if the
	 * connection is not properly started via {@link Connection#start()} and a message is received
	 * by a consumer. It is a very common gotcha to forget to <a
	 * href="http://activemq.apache.org/i-am-not-receiving-any-messages-what-is-wrong.html">start
	 * the connection</a> so this option makes the default case to create a warning if the user
	 * forgets. To disable the warning just set the value to < 0 (say -1).
	 */
	 ...
	 * with frameworks which don't expose the Connection such as Spring JmsTemplate, you can still
	 * register a transport listener.
	 * @param transportListener sets the listener to be registered on all connections created by
	 *            this factory
	 */
	...
	/**
	 * Allows an {@link ExceptionListener} to be configured on the ConnectionFactory so that when
	 * this factory is used by frameworks which don't expose the Connection such as Spring
	 * JmsTemplate, you can register an exception listener.
	 * <p>
	 * Note: access to this exceptionLinstener will <b>not</b> be serialized if it is associated
	 * with more than on connection (as it will be if more than one connection is subsequently
	 * created by this connection factory)
	 ...
	 * so that when this factory is used by frameworks which don't expose the Connection such as
	 * Spring JmsTemplate, you can register an exception listener.
	 * <p>
	 * Note: access to this clientInternalExceptionListener will <b>not</b> be serialized if it is
	 * associated with more than on connection (as it will be if more than one connection is
	 * subsequently created by this connection factory)
	 * @param clientInternalExceptionListener sets the exception listener to be registered on all
	 ...
	 * rollback Used in conjunction with
	...
	/**
	 * When true a MessageConsumer will not stop Message delivery before re-delivering Messages from
	 * a rolled back transaction. This implies that message order will not be preserved and also
	 * will result in the TransactedIndividualAck option to be enabled.
	 */
	 ...
	 * optimizedAcknowledge enabled to send an ack for any outstanding Message Acks. By default this
	 * value is set to zero meaning that the consumers will not do any background Message
	 * acknowledgment.
	...
	/**
	 * Sets the amount of time between scheduled sends of any outstanding Message Acks for consumers
	 * that have been configured with optimizeAcknowledge enabled.
	...
	/**
	 * uses the connection id as the resource identity for XAResource.isSameRM ensuring join will
	 * only occur on a single connection
	 */
	...
	/**
	 * Controls whether message expiration checking is done in each MessageConsumer prior to
	 * dispatching a message. Disabling this check can lead to consumption of expired messages.
	 * @param consumerExpiryCheckEnabled controls whether expiration checking is done prior to
	 *            dispatch.
	 */
