...
public class StartCommand extends AbstractCommand {
	...
	protected void runTask(List<String> brokerURIs) throws Exception {
		...
		while (true) {
			...
			try {
				// brokerURIs表示需要通过哪一种方式创建系统中所需要的对象，如果brokerURIs为empty，那么就使用默认方式，默认方式是xbean:activemq.xml
				...
				// configURI的值是xbean:activemq.xml，通过对activemq.xml文件的处理，下面的方法获得BrokerService对象
				broker = BrokerFactory.createBroker(configURI);
				// 在BrokerService对象未启动的情况下，会启动
				// 下面从这里开始看
				broker.start();
			} catch (Exception e) {
