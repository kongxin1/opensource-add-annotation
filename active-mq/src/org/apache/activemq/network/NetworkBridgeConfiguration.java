 ...
 */
/**
 * 这个类中基本都是一些属性的配置代码，基本没有具体的逻辑代码，可以看做是一个POJO
 * @ClassName: NetworkBridgeConfiguration
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月4日 下午9:59:52
 */
public class NetworkBridgeConfiguration {
	 ...
	 */
	/**
	 * 返回当前该对象中的Destination过滤器
	 * @Title: getDestinationFilter
	 * @Description: TODO
	 * @return
	 * @return: String
	 */
	...
	/**
	 * @param val if true, duplicate network queue subscriptions (in a cyclic network) will be
	 *            suppressed
	 */
	...
	/**
	 * @param val if true, duplicate network topic subscriptions (in a cyclic network) will be
	 *            suppressed
	 */
	...
	/**
	 * @param alwaysSyncSend when true, both persistent and non persistent messages will be sent
	 *            using a request. When false, non persistent messages are acked once the oneway
	 *            send succeeds, which can potentially lead to message loss. Using an async request,
	 *            allows multiple outstanding requests. This ensures that a bridge need not block
	 *            all sending when the remote broker needs to flow control a single destination.
	 */
	...
	/**
	 * @param consumerPriorityBase , default -5. Sets the starting priority for consumers. This base
	 *            value will be decremented by the length of the broker path when
	 *            decreaseNetworkConsumerPriority is set.
	 */
	...
	/**
	 * @param useCompression True if the Network should enforce compression for messages sent.
	 */
