 ...
 * <LI>It provides a way to create <CODE>Queue</CODE> or <CODE>Topic</CODE> objects for those
 * clients that need to dynamically manipulate provider-specific destination names.
 * <LI>It supports a single series of transactions that combine work spanning its producers and
 * consumers into atomic units.
 ...
 * <P>
 * One typical use is to have a thread block on a synchronous <CODE>MessageConsumer</CODE> until a
 * message arrives. The thread may then use one or more of the <CODE>Session</CODE>'s
 * <CODE>MessageProducer</CODE>s.
 * <P>
 * If a client desires to have one thread produce messages while others consume them, the client
 * should use a separate session for its producing thread.
 * <P>
 * Once a connection has been started, any session with one or more registered message listeners is
 * dedicated to the thread of control that delivers messages to it. It is erroneous for client code
 * to use this session or any of its constituent objects from another thread of control. The only
 * exception to this rule is the use of the session or connection <CODE>close</CODE> method.
 * <P>
 * It should be easy for most clients to partition their work naturally into sessions. This model
 * allows clients to start simply and incrementally add message processing complexity as their need
 * for concurrency grows.
 * <P>
 * The <CODE>close</CODE> method is the only session method that can be called while some other
 * session method is being executed in another thread.
 * <P>
 * A session may be specified as transacted. Each transacted session supports a single series of
 * transactions. Each transaction groups a set of message sends and a set of message receives into
 * an atomic unit of work. In effect, transactions organize a session's input message stream and
 * output message stream into series of atomic units. When a transaction commits, its atomic unit of
 * input is acknowledged and its associated atomic unit of output is sent. If a transaction rollback
 * is done, the transaction's sent messages are destroyed and the session's input is automatically
 * recovered.
 * <P>
 * The content of a transaction's input and output units is simply those messages that have been
 * produced and consumed within the session's current transaction.
 * <P>
 * A transaction is completed using either its session's <CODE>commit</CODE> method or its session's
 * <CODE>rollback </CODE> method. The completion of a session's current transaction automatically
 * begins the next. The result is that a transacted session always has a current transaction within
 * which its work is done.
 * <P>
 * The Java Transaction Service (JTS) or some other transaction monitor may be used to combine a
 * session's transaction with transactions on other resources (databases, other JMS sessions, etc.).
 * Since Java distributed transactions are controlled via the Java Transaction API (JTA), use of the
 * session's <CODE>commit</CODE> and <CODE>rollback</CODE> methods in this context is prohibited.
 * <P>
 * The JMS API does not require support for JTA; however, it does define how a provider supplies
 * this support.
 * <P>
 * Although it is also possible for a JMS client to handle distributed transactions directly, it is
 * unlikely that many JMS clients will do this. Support for JTA in the JMS API is targeted at
 * systems vendors who will be integrating the JMS API into their application server products.
...
public class ActiveMQSession implements Session, QueueSession, TopicSession, StatsCapable, ActiveMQDispatcher {
	/**
	 * Only acknowledge an individual message - using message.acknowledge() as opposed to
	 * CLIENT_ACKNOWLEDGE which acknowledges all messages consumed by a session at when
	 ...
	 * @param connection
	 * @param sessionId
	 * @param acknowledgeMode n.b if transacted - the acknowledgeMode == Session.SESSION_TRANSACTED
	 * @param asyncDispatch
	 ...
	 * @param transactionContext - provides the means to control a JMS transaction.
	 */
	...
	/**
	 * Creates a <CODE>BytesMessage</CODE> object. A <CODE>BytesMessage</CODE> object is used to
	 * send a message containing a stream of uninterpreted bytes.
	...
	/**
	 * Creates a <CODE>MapMessage</CODE> object. A <CODE>MapMessage</CODE> object is used to send a
	 * self-defining set of name-value pairs, where names are <CODE>String</CODE> objects and values
	 * are primitive values in the Java programming language.
	...
	/**
	 * Creates a <CODE>Message</CODE> object. The <CODE>Message</CODE> interface is the root
	 * interface of all JMS messages. A <CODE>Message</CODE> object holds all the standard message
	 * header information. It can be sent when a message containing only header information is
	 * sufficient.
	...
	/**
	 * Creates an <CODE>ObjectMessage</CODE> object. An <CODE>ObjectMessage</CODE> object is used to
	...
	/**
	 * Creates a <CODE>TextMessage</CODE> object. A <CODE>TextMessage</CODE> object is used to send
	 * a message containing a <CODE>String</CODE> object.
	...
	/**
	 * Creates an initialized <CODE>BlobMessage</CODE> object. A <CODE>BlobMessage</CODE> object is
	 * used to send a message containing a <CODE>URL</CODE> which points to some network addressible
	 * BLOB.
	 * @param url the network addressable URL used to pass directly to the consumer
	 ...
	 * BLOB.
	 * @param url the network addressable URL used to pass directly to the consumer
	 * @param deletedByBroker indicates whether or not the resource is deleted by the broker when
	...
	/**
	 * Creates an initialized <CODE>BlobMessage</CODE> object. A <CODE>BlobMessage</CODE> object is
	 * used to send a message containing the <CODE>File</CODE> content. Before the message is sent
	 * the file conent will be uploaded to the broker or some other remote repository depending on
	 * the {@link #getBlobTransferPolicy()}.
	 * @param file the file to be uploaded to some remote repo (or the broker) depending on the
	 *            strategy
	 * @return a BlobMessage
	 * @throws JMSException if the JMS provider fails to create this message due to some internal
	 *             error.
	 ...
	 * used to send a message containing the <CODE>File</CODE> content. Before the message is sent
	 * the file conent will be uploaded to the broker or some other remote repository depending on
	 * the {@link #getBlobTransferPolicy()}. <br/>
	 * <p>
	 * The caller of this method is responsible for closing the input stream that is used, however
	 * the stream can not be closed until <b>after</b> the message has been sent. To have this class
	 * manage the stream and close it automatically, use the method
	 * {@link ActiveMQSession#createBlobMessage(File)}
	 * @param in the stream to be uploaded to some remote repo (or the broker) depending on the
	 *            strategy
	 * @return a BlobMessage
	 * @throws JMSException if the JMS provider fails to create this message due to some internal
	 *             error.
	 ...
	 * that the session is created. If the session is transacted, the acknowledgement mode is
	 * ignored.
	 ...
	 * <P>
	 * This call will block until a <CODE>receive</CODE> call or message listener in progress has
	 * completed. A blocked message consumer <CODE>receive</CODE> call returns <CODE>null</CODE>
	 ...
	 * <CODE> JMSException.IllegalStateException</CODE>. Closing a closed session must <I>not </I>
	 * throw an exception.
	 * @throws JMSException if the JMS provider fails to close the session due to some internal
	 *             error.
	 */
	@Override
	...
	/**
	 * Check if the session is closed. It is used for ensuring that the session is open before
	 ...
	 * </UL>
	 * @throws JMSException if the JMS provider fails to stop and restart message delivery due to
	 *             some internal error.
	 ...
	 * @throws JMSException if the JMS provider fails to get the message listener due to an internal
	 *             error.
	 ...
	 * <P>
	 * If this session has been closed, then an {@link IllegalStateException} is thrown, if trying
	 * to set a new listener. However setting the listener to <tt>null</tt> is allowed, to clear the
	 * listener, even if this session has been closed prior.
	 * <P>
	 * This is an expert facility not used by regular JMS clients.
	 * @param listener the message listener to associate with this session
	 * @throws JMSException if the JMS provider fails to set the message listener due to an internal
	 *             error.
	...
	public void run() {
		...
		while ((messageDispatch = executor.dequeueNoWait()) != null) {
			...
			/*
			 * The redelivery guard is to allow the endpoint lifecycle to complete before the
			 * messsage is dispatched. We dont want the after deliver being called after the
			 * redeliver as it may cause some weird stuff.
			 */
			synchronized (redeliveryGuard) {
				try {
					...
					if (ack.getTransactionId() != null) {
						getTransactionContext().addSynchronization(new Synchronization() {
							final int clearRequestCount = (clearRequestsCounter.get() == Integer.MAX_VALUE ? clearRequestsCounter
									.incrementAndGet() : clearRequestsCounter.get());
							...
							public void beforeEnd() throws Exception {
								...
								if (ack.getTransactionId().isXATransaction()
										&& !connection.hasDispatcher(ack.getConsumerId())) {
							...
							public void afterRollback() throws Exception {
								...
								// don't redeliver if we have been interrupted b/c the broker will
								// redeliver on reconnect
								if (clearRequestsCounter.get() > clearRequestCount) {
									LOG.debug("No redelivery of {} on rollback of {} due to failover of {}", md,
								...
								if (ack.getTransactionId().isXATransaction()
									...
									LOG.debug(
											...
											md, ack.getTransactionId(), connection.getTransport());
									return;
								}
								...
								if (redeliveryPolicy.getMaximumRedeliveries() != RedeliveryPolicy.NO_MAXIMUM_REDELIVERIES
									...
									ack.setFirstMessageId(md.getMessage().getMessageId());
									ack.setPoisonCause(new Throwable("Exceeded ra redelivery policy limit:"
											+ redeliveryPolicy));
									asyncSendPacket(ack);
								} else {
									...
									/*
									 * If we are a non blocking delivery then we need to stop the
									 * executor to avoid more messages being delivered, once the
									 * message is redelivered we can restart it.
									 */
									...
									connection.getScheduler().executeAfterDelay(new Runnable() {
										...
										public void run() {
											/*
											 * wait for the first delivery to be complete, i.e.
											 * after delivery has been called.
											 */
											synchronized (redeliveryGuard) {
												/*
												 * If its non blocking then we can just dispatch in
												 * a new session.
												 */
												if (connection.isNonBlockingRedelivery()) {
													...
													/*
													 * If there has been an error thrown during
													 * afterDelivery then the endpoint will be
													 * marked as dead so redelivery will fail (and
													 * eventually the session marked as stale), in
													 * this case we can only call dispatch which
													 * will create a new session with a new
													 * endpoint.
													 */
			...
			/*
			 * this can be outside the try/catch as if an exception is thrown then this session will
			 * be marked as stale anyway. It also needs to be outside the redelivery guard.
			 */
			try {
	 ...
	 * <P>
	 * A client uses a <CODE>MessageProducer</CODE> object to send messages to a destination. Since
	 * <CODE>Queue </CODE> and <CODE>Topic</CODE> both inherit from <CODE>Destination</CODE>, they
	 * can be used in the destination parameter to create a <CODE>MessageProducer</CODE> object.
	 * @param destination the <CODE>Destination</CODE> to send to, or null if this is a producer
	...
	/**
	 * Creates a <CODE>MessageConsumer</CODE> for the specified destination. Since
	 * <CODE>Queue</CODE> and <CODE> Topic</CODE> both inherit from <CODE>Destination</CODE>, they
	 * can be used in the destination parameter to create a <CODE>MessageConsumer</CODE>.
	...
	/**
	 * Creates a <CODE>MessageConsumer</CODE> for the specified destination, using a message
	 * selector. Since <CODE> Queue</CODE> and <CODE>Topic</CODE> both inherit from
	 * <CODE>Destination</CODE>, they can be used in the destination parameter to create a
	 * <CODE>MessageConsumer</CODE>.
	 * <P>
	 * A client uses a <CODE>MessageConsumer</CODE> object to receive messages that have been sent
	 * to a destination.
	 * @param destination the <CODE>Destination</CODE> to access
	 * @param messageSelector only messages with properties matching the message selector expression
	 *            are delivered. A value of null or an empty string indicates that there is no
	...
	/**
	 * Creates <CODE>MessageConsumer</CODE> for the specified destination, using a message selector.
	 * This method can specify whether messages published by its own connection should be delivered
	 * to it, if the destination is a topic.
	 * <P>
	 * Since <CODE>Queue</CODE> and <CODE>Topic</CODE> both inherit from <CODE>Destination</CODE>,
	 * they can be used in the destination parameter to create a <CODE>MessageConsumer</CODE>.
	 * <P>
	 * A client uses a <CODE>MessageConsumer</CODE> object to receive messages that have been
	 * published to a destination.
	 * <P>
	 * In some cases, a connection may both publish and subscribe to a topic. The consumer
	 * <CODE>NoLocal</CODE> attribute allows a consumer to inhibit the delivery of messages
	 * published by its own connection. The default value for this attribute is False. The
	 * <CODE>noLocal</CODE> value must be supported by destinations that are topics.
	 * @param destination the <CODE>Destination</CODE> to access
	 * @param messageSelector only messages with properties matching the message selector expression
	 *            are delivered. A value of null or an empty string indicates that there is no
	 *            message selector for the message consumer.
	 * @param noLocal - if true, and the destination is a topic, inhibits the delivery of messages
	 *            published by its own connection. The behavior for <CODE>NoLocal</CODE> is not
	...
	public MessageConsumer createConsumer(Destination destination, String messageSelector, boolean noLocal,
		...
		return new ActiveMQMessageConsumer(this, getNextConsumerId(), activemqDestination, null, messageSelector,
				prefetch, prefetchPolicy.getMaximumPendingMessageLimit(), noLocal, false, isAsyncDispatch(),
				messageListener);
	}
	/**
	 * Creates a queue identity given a <CODE>Queue</CODE> name.
	 * <P>
	 * This facility is provided for the rare cases where clients need to dynamically manipulate
	 * queue identity. It allows the creation of a queue identity with a provider-specific name.
	 * Clients that depend on this ability are not portable.
	 * <P>
	 * Note that this method is not for creating the physical queue. The physical creation of queues
	 * is an administrative task and is not to be initiated by the JMS API. The one exception is the
	 ...
	 * <P>
	 * This facility is provided for the rare cases where clients need to dynamically manipulate
	 * topic identity. This allows the creation of a topic identity with a provider-specific name.
	 * Clients that depend on this ability are not portable.
	 * <P>
	 * Note that this method is not for creating the physical topic. The physical creation of topics
	 * is an administrative task and is not to be initiated by the JMS API. The one exception is the
	 ...
	 * published while the subscriber is inactive, it uses a durable <CODE>TopicSubscriber</CODE>.
	 * The JMS provider retains a record of this durable subscription and insures that all messages
	 * from the topic's publishers are retained until they are acknowledged by this durable
	 * subscriber or they have expired.
	 * <P>
	 * Sessions with durable subscribers must always provide the same client identifier. In
	 ...
	 * <CODE>TopicSubscriber</CODE> with the same name and a new topic and/or message selector.
	 ...
	 * <CODE>NoLocal</CODE> attribute allows a subscriber to inhibit the delivery of messages
	 * published by its own connection. The default value for this attribute is false.
	...
	/**
	 * Creates a durable subscriber to the specified topic, using a message selector and specifying
	 ...
	 * <P>
	 * Sessions with durable subscribers must always provide the same client identifier. In
	 * addition, each client must specify a name which uniquely identifies (within client
	 * identifier) each durable subscription it creates. Only one session at a time can have a
	 * <CODE>TopicSubscriber</CODE> for a particular durable subscription. An inactive durable
	 * subscriber is one that exists but does not currently have a message consumer associated with
	 * it.
	 * <P>
	 * A client can change an existing durable subscription by creating a durable
	 * <CODE>TopicSubscriber</CODE> with the same name and a new topic and/or message selector.
	...
	public TopicSubscriber createDurableSubscriber(Topic topic, String name, String messageSelector, boolean noLocal)
		...
		return new ActiveMQTopicSubscriber(this, getNextConsumerId(),
				ActiveMQMessageTransformation.transformDestination(topic), name, messageSelector, prefetch,
				maxPrendingLimit, noLocal, false, asyncDispatch);
	}
	/**
	 * Creates a <CODE>QueueBrowser</CODE> object to peek at the messages on the specified queue.
	 * @param queue the <CODE>queue</CODE> to access
	...
	public QueueBrowser createBrowser(Queue queue, String messageSelector) throws JMSException {
		checkClosed();
		return new ActiveMQQueueBrowser(this, getNextConsumerId(),
				ActiveMQMessageTransformation.transformDestination(queue), messageSelector, asyncDispatch);
	}
	/**
	 * Creates a <CODE>TemporaryQueue</CODE> object. Its lifetime will be that of the
	 * <CODE>Connection</CODE> unless it is deleted earlier.
	...
	public QueueReceiver createReceiver(Queue queue, String messageSelector) throws JMSException {
		...
		ActiveMQPrefetchPolicy prefetchPolicy = this.connection.getPrefetchPolicy();
		return new ActiveMQQueueReceiver(this, getNextConsumerId(),
				ActiveMQMessageTransformation.transformDestination(queue), messageSelector,
				prefetchPolicy.getQueuePrefetch(), prefetchPolicy.getMaximumPendingMessageLimit(), asyncDispatch);
	}
	/**
	 ...
	 * <p/>
	 * <P>
	 * A client uses a <CODE>TopicSubscriber</CODE> object to receive messages that have been
	 * published to a topic.
	 * <p/>
	 * <P>
	 * Regular <CODE>TopicSubscriber</CODE> objects are not durable. They receive only messages that
	...
	/**
	 * Creates a nondurable subscriber to the specified topic, using a message selector or
	 ...
	 * <p/>
	 * <P>
	 * Messages filtered out by a subscriber's message selector will never be delivered to the
	...
	public TopicSubscriber createSubscriber(Topic topic, String messageSelector, boolean noLocal) throws JMSException {
		...
		ActiveMQPrefetchPolicy prefetchPolicy = this.connection.getPrefetchPolicy();
		return new ActiveMQTopicSubscriber(this, getNextConsumerId(),
				ActiveMQMessageTransformation.transformDestination(topic), null, messageSelector,
				prefetchPolicy.getTopicPrefetch(), prefetchPolicy.getMaximumPendingMessageLimit(), noLocal, false,
				asyncDispatch);
	}
	/**
	 ...
	 * <p/>
	 * <P>
	 * A client uses a <CODE>TopicPublisher</CODE> object to publish messages on a topic. Each time
	 * a client creates a <CODE>TopicPublisher</CODE> on a topic, it defines a new sequence of
	 * messages that have no ordering relationship with the messages it has previously sent.
	 ...
	 * <CODE>MessageConsumer </CODE> or <CODE>TopicSubscriber</CODE> for the subscription, or while
	 * a consumed message is part of a pending transaction or has not been acknowledged in the
	 * session.
	 * @param name the name used to identify this subscription
	 ...
	 * <P>
	 * All consumed JMS messages support the <CODE>acknowledge</CODE> method for use when a client
	 * has specified that its JMS session's consumed messages are to be explicitly acknowledged. By
	 * invoking <CODE>acknowledge</CODE> on a consumed message, a client acknowledges all messages
	 * consumed by the session that the message was delivered to.
	 * <P>
	 * Calls to <CODE>acknowledge</CODE> are ignored for both transacted sessions and sessions
	 ...
	 * @throws JMSException if the JMS provider fails to acknowledge the messages due to some
	 *             internal error.
	...
	/**
	 * Sets the transformer used to transform messages before they are sent on to the JMS bus or
	 * when they are received from the bus but before they are delivered to the JMS client
	 */
	...
	/**
	 * Sets the policy used to describe how out-of-band BLOBs (Binary Large OBjects) are transferred
	 * from producers to brokers to consumers
	 */
	...
	/**
	 * highest sequence id of the last message delivered by this session. Passed to the broker in
	 * the close command, maintained by dispose()
