...
/**
 * Policy object that controls how a TransportConnector publishes the connector's address to the
 * outside world. By default the connector will publish itself using the resolved host name of the
 * bound server socket.
 * @org.apache.xbean.XBean
 */
/**
 * 将主机信息发布的策略类，可以根据这里面的方法发布要求的信息
 * @ClassName: PublishedAddressPolicy
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月16日 下午5:04:04
 */
public class PublishedAddressPolicy {
	...
	public enum PublishedHostStrategy {
		DEFAULT, IPADDRESS, HOSTNAME, FQDN;
	...
	/**
	 * Using the supplied TransportConnector this method returns the String that will be used to
	 * update clients with this connector's connect address.
	 * @param connector The TransportConnector whose address is to be published.
	...
	/**
	 * Subclasses can override what host value is published by implementing alternate logic for this
	 * method.
	 * @param uriHostEntry
	 ...
	 */
	/**
	 * 根据需要发布的主机信息策略不同，将uri转化为要求的信息后返回
	 * @Title: getPublishedHostValue
	 * @Description: TODO
	 * @param uriHostEntry
	 * @return
	 * @throws UnknownHostException
	...
	/**
	 * Subclasses can override what path value is published by implementing alternate logic for this
	 * method. By default this method simply returns what was already set as the Path value in the
	 * original URI.
	 * @param uriPathEntry The original value of the URI path.
	...
	/**
	 * Subclasses can override what host value is published by implementing alternate logic for this
	 * method. By default this method simply returns what was already set as the Fragment value in
	 * the original URI.
	 * @param uriFragmentEntry The original value of the URI Fragment.
	...
	/**
	 * Subclasses can override what user info value is published by implementing alternate logic for
	 * this method. By default this method simply returns what was already set as the UserInfo value
	 * in the original URI.
	 * @param uriUserInfoEntry The original value of the URI user info.
	...
	/**
	 * Gets the URI query that's configured on the published URI that's sent to client's when the
	 * cluster info is updated.
	...
	/**
	 * Sets the URI query that's configured on the published URI that's sent to client's when the
	 * cluster info is updated.
