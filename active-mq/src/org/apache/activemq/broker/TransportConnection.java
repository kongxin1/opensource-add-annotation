...
/**
 * 这个对象是处理消息的核心类，方法以process开头方法就是处理消息的方法。当有一个客户端连接服务器时，就会创建一个这个对象，这个对象是在TransportConnector类中创建的
 * @ClassName: TransportConnection
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月22日 下午9:32:19
 */
public class TransportConnection implements Connection, Task, CommandVisitor {
	...
	// 下面的这个属性中会保存有服务器端的所有与客户端建立的连接，每一个连接就是一个transportconnection对象
	...
	private final Transport transport;// 对于TCP服务，这个属性持有一个封装了TcpTransport对象的对象
	...
	// ProducerBrokerExchange对象中记录了生产者的当前信息，有当前生产者处理的最后一个id，ConnectionContext对象和生产者状态对象等信息
	...
	// 客户端与服务器之间的连接可以建立多个，当传输了wireformatinfo对象后，本transportconnection对象就已经建立了，之后再创建的连接都会存储在下面的属性中
	...
	public Response processAddConsumer(ConsumerInfo info) throws Exception {
		// 如果ConsumerId中没有设置parentId的话，那么parentId就是自己
		SessionId sessionId = info.getConsumerId().getParentId();
		// 同上，如果为null，还是自己
		ConnectionId connectionId = sessionId.getParentId();
		// 找到connectionId对象的对应的TransportConnectionState对象，这个在处理ConnectionInfo时，已经加入进去了
		TransportConnectionState cs = lookupConnectionState(connectionId);
		if (cs == null) {
			throw new IllegalStateException("Cannot add a consumer to a connection that had not been registered: "
					+ connectionId);
		}
		// 这个在处理ConnectionInfo时，已经加入进去了，里面记录了一个最重要的信息就是ConnectionId的字符串，在创建TransportConnectionState对象时，会自动创建一个SessionState对象
		SessionState ss = cs.getSessionState(sessionId);
		if (ss == null) {
		...
		// Avoid replaying dup commands
		// 避免处理相同的命令对象，如果处理过就不再处理
		if (!ss.getConsumerIds().contains(info.getConsumerId())) {
			// 在客户端传递到服务端的信息中会带有一个destination属性，这个属性指定了实现类
			ActiveMQDestination destination = info.getDestination();
			// 检查该connectionid下一共有多少个consumer，如果超过最大值，就抛出异常
			...
			// 创建出订阅者，并将订阅者加入到Topic对象或者Queue中
			broker.addConsumer(cs.getContext(), info);
			try {
				// 将consumerinfo加入到sessionstate中
				ss.addConsumer(info);
				// 创建ConsumerBrokerExchange对象，并将对象加入到consumerExchanges中。
	...
	public Response processAddSession(SessionInfo info) throws Exception {
		// info中的sessionid在客户端传输到服务器的数据中已经包含了
		ConnectionId connectionId = info.getSessionId().getParentId();
		// 根据connectionid找到对应的状态对象
		TransportConnectionState cs = lookupConnectionState(connectionId);
		// Avoid replaying dup commands
		if (cs != null && !cs.getSessionIds().contains(info.getSessionId())) {
			// 对于regionbroker来说，下面这个方法是一个空实现
	...
	public Response processRemoveSession(SessionId id, long lastDeliveredSequenceId) throws Exception {
		...
		cs.removeSession(id);
		broker.removeSession(cs.getContext(), session.getInfo());// 空实现
		return null;
	}
	@Override
	/**
	 * 当客户端调用connection.start方法时，会调用这个方法。当服务端处理ConnectionInfo时，会调用这个方法，这个方法主要是对客户端和服务端的一些信息进行保存存储到指定的几个属性中：brokerConnectionStates（记录当前的连接状态信息）、将clientId和ConnectionContext对象保存到clientIdSet集合中以及将ConnectionContext保存到connections集合中（这是在RegionBroker中做的）
	 */
	public Response processAddConnection(ConnectionInfo info) throws Exception {
		...
		// 下面这个操作主要是为防止客户端同时发起两个并发请求，对于两个并发请求只有一个请求可以创建TransportConnectionState对象
		synchronized (brokerConnectionStates) {
			// 如果state=null，说明客户端当前没有连接过本服务器
		...
		synchronized (state.getConnectionMutex()) {
			// 除非子类对brokerConnectionStates做了修改，注入了不是本对象的其他对象，否则我认为下面这一步没有必要
		...
		// 将入参注入到TransportConnectionStateRegister里面
	...
	/**
	 * 当对ConnectionControl处理时，下面这个方法基本是一个空实现
	...
	protected void processDispatch(Command command) throws IOException {
		// 当处理的COmmand对象是ConnectionControl时，command.isMessageDispatch()返回false
	...
	protected void doStop() throws Exception {
		...
		connector.onStopped(this);// 将本连接对象从connector对象的连接集合中删除
		try {
			synchronized (this) {
	...
	/**
	 * 根据入参得到ProducerBrokerExchange对象，如果不存在就创建一个新的
	...
	private ProducerBrokerExchange getProducerBrokerExchange(ProducerId id) throws IOException {
		...
		if (result == null) {
			synchronized (producerExchanges) {
				...
				if (ss != null) {
					// 在处理ProducerInfo对象时，ProducerState对象已经设置进去了
	...
	/**
	 * 将入参注册到TransportConnectionStateRegister里面
	...
	/**
	 * 在connectionStateRegister对象中搜寻ConnectionId对象的TransportConnectionState对象
	...
	/**
	 * 计算在同一个ConnectionId下一共有多少个consumerid
