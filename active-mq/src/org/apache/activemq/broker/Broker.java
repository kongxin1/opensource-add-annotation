...
/**
 * The Message Broker which routes messages, maintains subscriptions and connections, acknowledges
 * messages and handles transactions.
 */
/**
 * 处理消息的核心，可以路由消息
 * @ClassName: Broker
...
public interface Broker extends Region, Service {
	 ...
	 * @param info
	 * @param error null if the client requested the disconnect or the error that caused the client
	 *            to disconnect.
	 * @throws Exception TODO
	 */
	...
	/**
	 * @return the connection context used to make administration operations on startup or via JMX
	 *         MBeans
	 */
	ConnectionContext getAdminConnectionContext();
	/**
	 * Sets the default administration connection context used when configuring the broker on
	 * startup or via JMX
	...
	Broker getRoot();
	/**
	 * Determine if a message has expired -allows default behaviour to be overriden - as the
	 * timestamp set by the producer can be out of sync with the broker
	 ...
	 */
	boolean sendToDeadLetterQueue(ConnectionContext context, MessageReference messageReference,
