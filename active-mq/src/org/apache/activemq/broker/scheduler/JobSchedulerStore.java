 ...
 */
/**
 * 本接口用来管理延迟处理消息的相关工作，同时还是一个调度器的集合，也就是若干个工作调度器会存储在本接口中，由本接口进行管理
 * @ClassName: JobSchedulerStore
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月9日 上午9:01:45
 */
public interface JobSchedulerStore extends Service {
	/**
	 * Gets the location where the Job Scheduler will write the persistent data used to preserve and
	 * recover scheduled Jobs. If the scheduler implementation does not utilize a file system based
	 * store this method returns null.
	 * @return the directory where persistent store data is written.
	 */
	File getDirectory();
	/**
	 * Sets the directory where persistent store data will be written. This method must be called
	 * before the scheduler store is started to have any effect.
	 * @param directory The directory where the job scheduler store is to be located.
	 */
	 ...
	 * @param name the name of the JobScheduler instance to lookup.
	 ...
	 * @param name the name of the scheduler instance to remove.
