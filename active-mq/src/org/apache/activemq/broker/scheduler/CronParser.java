 ...
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月11日 下午8:58:17
 */
public class CronParser {
	// 表示cron命令拆分后形成5部分
	private static final int NUMBER_TOKENS = 5;
	// 下面的每一个常量对应的是cron命令中的第几部分
	...
	/**
	 * 解析cron命令，获得大于等于当前时间的下一个执行时间，如果cron命令没有设置，那么返回0
	 * @Title: getNextScheduledTime
	 * @Description: TODO
	 * @param cronEntry
	 * @param currentTime
	 * @return
	...
	public static long getNextScheduledTime(final String cronEntry, long currentTime) throws MessageFormatException {
		...
		// * * * * *：表示每一分钟执行一次
		if (cronEntry.equals("* * * * *")) {
			result = currentTime + 60 * 1000;// 60 * 1000表示一分钟
			result = result / 60000 * 60000;// result表示在下一分钟执行，当然这个值是一个近似值
			return result;
		}
		// 获得拆分成5部分后的命令集合
