...
public abstract class BaseDestination implements Destination {
	...
	private boolean alwaysRetroactive = false;// 猜测应该是指消息可以追溯
	...
	public void messageDelivered(ConnectionContext context, MessageReference messageReference) {
		if (advisoryForDelivery) {
			broker.messageDelivered(context, messageReference);// 空实现
		}
	}
	/**
	 * Called when a message is discarded - e.g. running low on memory This will happen only if the
	 * policy is enabled - e.g. non durable topics
	 * @param context
	 * @param messageReference
	 */
	@Override
	...
	/**
	 * Provides a hook to allow messages with no consumer to be processed in some way - such as to
	 * send to a dead letter queue or something..
	 */
	/**
	 * 在没有订阅者消费消息时，会调用下面的这个方法，当消息满足一定的条件后，会将消息发送到别的队列中
	 * @Title: onMessageWithNoConsumers
	 * @Description: TODO
	 * @param context
	 * @param msg
	 * @throws Exception
	...
	protected final void waitForSpace(ConnectionContext context, ProducerBrokerExchange producerBrokerExchange,
			Usage<?> usage, String warning) throws IOException, InterruptedException, ResourceAllocationException {
		waitForSpace(context, producerBrokerExchange, usage, 100, warning);
	}
	protected final void waitForSpace(ConnectionContext context, ProducerBrokerExchange producerBrokerExchange,
			Usage<?> usage, int highWaterMark, String warning) throws IOException, InterruptedException,
		...
		if (!context.isNetworkConnection() && systemUsage.getSendFailIfNoSpaceAfterTimeout() != 0) {
			if (!usage.waitForSpace(systemUsage.getSendFailIfNoSpaceAfterTimeout(), highWaterMark)) {
				getLog().debug("sendFailIfNoSpaceAfterTimeout expired, forcing exception on send, usage: {}: {}",
						usage, warning);
				throw new ResourceAllocationException(warning);
			}
		} else {
			...
			while (!usage.waitForSpace(1000, highWaterMark)) {
				...
				if (now >= nextWarn) {
					getLog().info("{}: {} (blocking for: {}s)",
							new Object[] { usage, warning, new Long(((now - start) / 1000)) });
	...
	public void duplicateFromStore(Message message, Subscription durableSub) {
		...
		try {
			...
			getLog().error("Failed to acknowledge duplicate message {} from {} with {}", message.getMessageId(),
					destination, messageAck);
		}
	}
