...
public class TopicSubscription extends AbstractSubscription {
	...
	public TopicSubscription(Broker broker, ConnectionContext context, ConsumerInfo info, SystemUsage usageManager)
			throws Exception {
	...
	public void add(MessageReference node) throws Exception {
		// 下面的方法会检测消息的生产者id，如果生产者id已经发送过，就不再处理该消息
		if (isDuplicate(node)) {
			return;
		}
		...
		synchronized (matchedListMutex) {
			...
			// any more messages to it as those messages can only be advisories generated in the
			// process,
			...
			if (!isFull() && matched.isEmpty()) {
				...
				if (info.getPrefetchSize() > 1 && matched.size() > info.getPrefetchSize()) {
					...
					if (!isSlowConsumer()) {
						LOG.warn("{}: has twice its prefetch limit pending, without an ack; it appears to be slow",
								toString());
					...
					while (active) {
						while (matched.isFull()) {
							if (getContext().getStopping().get()) {
								LOG.warn("{}: stopped waiting for space in pendingMessage cursor for: {}", toString(),
										node.getMessageId());
						...
						while (!matched.isEmpty() && matched.size() > maximumPendingMessages) {
							...
							if (messagesToEvict == 0) {
								LOG.warn(
										...
										new Object[] { destination, messageEvictionStrategy, list.size() });
								break;
							}
						}
					}
	...
	/**
	 * Occurs when a pull times out. If nothing has been dispatched since the timeout was setup,
	 * then send the NULL message.
	 */
		...
		synchronized (dispatchLock) {
			...
			for (final MessageReference node : removeList) {
				...
				if (info.isNetworkSubscription()) {
					((Destination) node.getRegionDestination()).getDestinationStatistics().getForwards()
	...
	public int countBeforeFull() {
		return getPrefetchSize() == 0 ? prefetchExtension.get() : info.getPrefetchSize() + prefetchExtension.get()
	...
	public int getDispatchedQueueSize() {
		return (int) (getSubscriptionStatistics().getDispatched().getCount() - prefetchExtension.get() - getSubscriptionStatistics()
	...
	/**
	 * @return the number of matched messages (messages targeted for the subscription but not yet
	 *         able to be dispatched due to the prefetch buffer being full).
	 */
	public int matched() {
		synchronized (matchedListMutex) {
			return matched.size();
	...
	/**
	 * Sets the maximum number of pending messages that can be matched against this consumer before
	...
	private void dispatch(final MessageReference node) throws IOException {
		...
		if (node != null) {
			node.incrementReferenceCount();// 增加消息引用次数
		}
	...
	private void discard(MessageReference message) {
		...
		try {
			...
			broker.getRoot().sendToDeadLetterQueue(getContext(), message, this,
					new Throwable("TopicSubDiscard. ID:" + info.getConsumerId()));
		} finally {
			discarding = false;
		}
	}
	...
	public String toString() {
		return "TopicSubscription:" + " consumer=" + info.getConsumerId() + ", destinations=" + destinations.size()
				+ ", dispatched=" + getDispatchedQueueSize() + ", delivered=" + getDequeueCounter() + ", matched="
				+ matched() + ", discarded=" + discarded();
	}
	@Override
	public void destroy() {
		this.active = false;
