...
/**
 * Represents an entry in a {@link PolicyMap} for assigning policies to a specific destination or a
 * hierarchical wildcard area of destinations.
...
public class PolicyEntry extends DestinationMapEntry {
	...
	public void configure(Broker broker, SystemUsage memoryManager, TopicSubscription subscription) {
		...
		if (pendingMessageLimitStrategy != null) {
			...
			if (value >= 0) {
				LOG.debug("Setting the maximumPendingMessages size to: {} for consumer: {}", value, subscription
						.getInfo().getConsumerId());
	...
	public void configure(Broker broker, SystemUsage memoryManager, DurableTopicSubscription sub) {
		...
		if (pendingDurableSubscriberPolicy != null) {
			PendingMessageCursor cursor = pendingDurableSubscriberPolicy.getSubscriberPendingMessageCursor(broker,
					clientId, subName, sub.getPrefetchSize(), sub);
	...
	/**
	 * Sends an advisory message if a non-persistent message is sent and there are no active
	 * consumers
	 */
	...
	/**
	 * Sets the strategy to calculate the maximum number of messages that are allowed to be pending
	 * on consumers (in addition to their prefetch sizes). Once the limit is reached, non-durable
	 * topics can then start discarding old messages. This allows us to keep dispatching messages to
	 * slow consumers while not blocking fast consumers and discarding the messages oldest first.
	 */
	...
	/**
	 * Sets the eviction strategy used to decide which message to evict when the slow consumer needs
	 * to discard messages
	 */
	...
	public MessageGroupMapFactory getMessageGroupMapFactory() {
		if (messageGroupMapFactory == null) {
			try {
				messageGroupMapFactory = GroupFactoryFinder
						.createMessageGroupMapFactory(getMessageGroupMapFactoryType());
			} catch (Exception e) {
	...
	/**
	 * Set's the interval at which warnings about producers being blocked by resource usage will be
	 * triggered. Values of 0 or less will disable warnings
	 * @param blockedProducerWarningInterval the interval at which warning about blocked producers
	 *            will be triggered.
	 */
	...
	/**
	 * @return the interval at which warning about blocked producers will be triggered.
	 */
	...
	/**
	 * Returns true if the original message body should be included when applicable for advisory
	 * messages
	 * @return
	 */
	 ...
	 * @param inactiveTimoutBeforeGC time in milliseconds to configure as the inactive timeout.
	 * @deprecated use getInactiveTimeoutBeforeGC instead.
	 */
	@Deprecated
