...
/**
 * The Topic is a destination that sends a copy of a message to every active Subscription
 * registered.
 */
public class Topic extends BaseDestination implements Task {
	...
	public void initialize() throws Exception {
		super.initialize();// 仅仅设置了一个store属性
	...
	public void addSubscription(ConnectionContext context, final Subscription sub) throws Exception {
		if (!sub.getConsumerInfo().isDurable()) {
			...
			if (sub.getConsumerInfo().isRetroactive() || isAlwaysRetroactive()) {
				...
				synchronized (consumers) {
					if (!consumers.contains(sub)) {
						sub.add(context, this);// 将目的地地址添加到subscription中
						consumers.add(sub);// 将订阅者添加到消费者集合中
						super.addSubscription(context, sub);
					}
				}
			}
	...
	public void deactivate(ConnectionContext context, DurableTopicSubscription sub, List<MessageReference> dispatched)
			throws Exception {
		synchronized (consumers) {
			consumers.remove(sub);
		}
	...
	public void send(final ProducerBrokerExchange producerExchange, final Message message) throws Exception {
		...
		producerExchange.incrementSend();// 发送数增加1
		final boolean sendProducerAck = !message.isResponseRequired() && producerInfo.getWindowSize() > 0
				&& !context.isInRecoveryMode();
		// There is delay between the client sending it and it arriving at the
		// destination.. it may have expired.
		if (message.isExpired()) {
			broker.messageExpired(context, message, null);// RegionBroker
		...
		if (memoryUsage.isFull()) {
			...
			if (isProducerFlowControl() && context.isProducerFlowControl()) {
				if (warnOnProducerFlowControl) {
					...
					LOG.info(
							"{}, Usage Manager memory limit reached {}. Producers will be throttled to the rate at which messages are removed from this destination to prevent flooding it. See http://activemq.apache.org/producer-flow-control.html for more info.",
				...
				if (!context.isNetworkConnection() && systemUsage.isSendFailIfNoSpace()) {
					throw new javax.jms.ResourceAllocationException("Usage Manager memory limit ("
							+ memoryUsage.getLimit() + ") reached. Rejecting send for producer ("
						...
						if (context.isInTransaction()) {
							...
							while (!memoryUsage.waitForSpace(1000)) {
								...
								if (count > 2 && context.isInTransaction()) {
									...
									LOG.warn(
											"Waiting for space to send transacted message - transaction elements = {} need more space to commit. Message = {}",
											size, message);
								}
								count++;
							}
	...
	/**
	 * do send the message - this needs to be synchronized to ensure messages are stored AND
	...
	synchronized void doMessageSend(final ProducerBrokerExchange producerExchange, final Message message)
		...
		final ConnectionContext context = producerExchange.getConnectionContext();
		message.setRegionDestination(this);// 设置消息的目的地
		message.getMessageId().setBrokerSequenceId(getDestinationSequenceId());// 设置消息的id值，调用的方法会生成下一个id
		...
		message.incrementReferenceCount();// 消息引用数加一，空间数增加
		if (context.isInTransaction()) {
			context.getTransaction().addSynchronization(new Synchronization() {
				...
				public void afterCommit() throws Exception {
					...
					if (broker.isExpired(message)) {
						getDestinationStatistics().getExpired().increment();// 统计过期的计数器增加
						broker.messageExpired(context, message, null);// 消息过期会发送到DeadLetterQueue
	...
	@Override
	/**
	 * 启动subscriptionRecoveryPolicy和memoryUsage对象，还需要启动一个定时程序
	 */
	public void start() throws Exception {
		this.subscriptionRecoveryPolicy.start();// 对master的标记信息，这个属性指的就是LastImageSubscriptionRecoveryPolicy
		if (memoryUsage != null) {
			memoryUsage.start();
		}
	...
	public void setSubscriptionRecoveryPolicy(SubscriptionRecoveryPolicy recoveryPolicy) {
		if (this.subscriptionRecoveryPolicy != null
				&& this.subscriptionRecoveryPolicy instanceof RetainedMessageSubscriptionRecoveryPolicy) {
	...
	protected void dispatch(final ConnectionContext context, Message message) throws Exception {
		...
		destinationStatistics.getEnqueues().increment();// 消息个数加1
		destinationStatistics.getMessageSize().addSize(message.getSize());// 增加消息的占用空间
		MessageEvaluationContext msgContext = null;
		dispatchLock.readLock().lock();
		try {
			// 对于master的标记消息，下面if的结果是false
			...
			// 将需要发送的消息存储到MessageEvaluationContext对象中
			msgContext = context.getMessageEvaluationContext();
			msgContext.setDestination(destination);
			msgContext.setMessageReference(message);
			// Topic对象中默认的分发策略是SimpleDispatchPolicy
