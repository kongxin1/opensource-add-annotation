...
/**
 * Creates <a href="http://activemq.org/site/virtual-destinations.html">Virtual Topics</a> using a
 * prefix and postfix. The virtual destination creates a wildcard that is then used to look up all
 * active queue subscriptions which match.
...
public class VirtualTopic implements VirtualDestination {
	...
	@Override
	/**
	 * 直接新建并返回一个和name名字一样的ActiveMQTopic对象
	 */
	...
	@Override
	/**
	 * 直接新建并返回一个和prefix+name+postfix名字一样的ActiveMQTopic对象
	 */
	...
	/**
	 * Indicates whether the selectors of consumers are used to determine dispatch to a virtual
	 * destination, when true only messages matching an existing consumer will be dispatched.
	...
	public String toString() {
		return new StringBuilder("VirtualTopic:").append(prefix).append(',').append(name).append(',').append(postfix)
				.append(',').append(selectorAware).append(',').append(local).toString();
	}
