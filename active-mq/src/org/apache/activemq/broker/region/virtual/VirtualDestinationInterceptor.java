...
/**
 * Implements <a href="http://activemq.apache.org/virtual-destinations.html">Virtual Topics</a>.
...
public class VirtualDestinationInterceptor implements DestinationInterceptor {
	...
	public Destination intercept(Destination destination) {
		...
		if (!mappedDestinations.isEmpty()) {
			...
			VirtualDestination virtualDestination = (VirtualDestination) mappedDestinations
					.toArray(new VirtualDestination[mappedDestinations.size()])[0];
			return virtualDestination.interceptMappedDestination(destination);
		}
		return destination;
	}
	@Override
	public synchronized void create(Broker broker, ConnectionContext context, ActiveMQDestination destination)
			throws Exception {
