...
/**
 * Creates <a href="http://activemq.org/site/mirrored-queues.html">Mirrored Queue</a> using a prefix
 * and postfix to define the topic name on which to mirror the queue to.
...
public class MirroredQueue implements DestinationInterceptor, BrokerServiceAware {
			...
			if (!destination.getActiveMQDestination().isTemporary() || brokerService.isUseTempMirroredQueues()) {
				try {
					...
					if (mirrorDestination != null) {
						// 通过下面这个对象可以将消息发送到镜像队列对应的topic中去
						return new DestinationFilter(destination) {
							public void send(ProducerBrokerExchange context, Message message) throws Exception {
								...
								message.setMemoryUsage(null); // set this to null so that it will
																// use the queue memoryUsage
																// instance instead of the topic.
	...
	/**
	 * Sets whether a copy of the message will be sent to each destination. Defaults to true so that
	 * the forward destination is set as the destination of the message
	 */
