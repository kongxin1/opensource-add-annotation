...
public class RegionBroker extends EmptyBroker {
	...
	protected final Map<ConnectionId, ConnectionState> connectionStates = Collections
	...
	// 将客户端与服务器端的连接对象保存下来，对于服务端的tcp协议来说，就是TransportConnection对象
	...
	// 属性中存放的是ActiveMQDestination与Destination的对应关系，其中后者是根据前者创建的
	 ...
	 * @throws IOException
	 */
	public RegionBroker(BrokerService brokerService, TaskRunnerFactory taskRunnerFactory, SystemUsage memoryManager,
			DestinationFactory destinationFactory, DestinationInterceptor destinationInterceptor, Scheduler scheduler,
	...
	public Map<ActiveMQDestination, Destination> getDestinationMap() {
		Map<ActiveMQDestination, Destination> answer = new HashMap<ActiveMQDestination, Destination>(getQueueRegion()
				.getDestinationMap());
	...
	protected Region createTempTopicRegion(SystemUsage memoryManager, TaskRunnerFactory taskRunnerFactory,
			DestinationFactory destinationFactory) {
		return new TempTopicRegion(this, destinationStatistics, memoryManager, taskRunnerFactory, destinationFactory);
	}
	protected Region createTempQueueRegion(SystemUsage memoryManager, TaskRunnerFactory taskRunnerFactory,
			DestinationFactory destinationFactory) {
		return new TempQueueRegion(this, destinationStatistics, memoryManager, taskRunnerFactory, destinationFactory);
	}
	protected Region createTopicRegion(SystemUsage memoryManager, TaskRunnerFactory taskRunnerFactory,
			DestinationFactory destinationFactory) {
		return new TopicRegion(this, destinationStatistics, memoryManager, taskRunnerFactory, destinationFactory);
	}
	protected Region createQueueRegion(SystemUsage memoryManager, TaskRunnerFactory taskRunnerFactory,
			DestinationFactory destinationFactory) {
	...
	@Override
	/**
	 * 主要做两件事：将clientId和ConnectionContext对象保存到clientIdSet集合中，将ConnectionContext保存到connections集合中
	 */
		...
		synchronized (clientIdSet) {
			...
			if (oldContext != null) {
				if (context.isAllowLinkStealing()) {
					...
					throw new InvalidClientIDException("Broker: " + getBrokerName() + " - Client: " + clientId
							+ " already connected from " + oldContext.getConnection().getRemoteAddress());
				}
			} else {
				clientIdSet.put(clientId, context);
			}
		...
		if (oldContext != null) {
			...
			if (oldContext.getConnection() != null) {
				...
				if (connection instanceof TransportConnection) {
					...
					transportConnection.stopAsync(new IOException("Stealing link for clientId " + clientId
							+ " From Connection " + oldContext.getConnection().getConnectionId()));
				} else {
					connection.stop();
				}
			} else {
	...
	@Override
	/**
	 *  从destinations属性中获取与入参对应的对象，如果找不到，就创建一个对应的Topic或者Queue对象，并放入destinations中
	 */
	public Destination addDestination(ConnectionContext context, ActiveMQDestination destination, boolean createIfTemp)
			throws Exception {
		Destination answer;
		...
		try {
			...
			// 得到一个和入参类型一样的Region实现，对于这个实现是在构造方法中设置好的，得到的是一个Topic或者Queue对象
	...
	@Override
	/**
	 * 消息发送，producerExchange保存有生产者信息，message保存有发送的地址和具体信息内容
	 * 消息发送是调用对应的Region对象发送的
	 */
	public void send(ProducerBrokerExchange producerExchange, Message message) throws Exception {
		// 获得消息的发送目的地，如果是master信息，目的地就是一个以字符串标示的ActiveMQTopic对象
		ActiveMQDestination destination = message.getDestination();
		// 设置消息时间
		message.setBrokerInTime(System.currentTimeMillis());
		// 对标记broker为master时，producerExchange.isMutable()返回true
		if (producerExchange.isMutable()
				|| producerExchange.getRegion() == null
				|| (producerExchange.getRegionDestination() != null && producerExchange.getRegionDestination()
						.isDisposed())) {
			// ensure the destination is registered with the RegionBroker
			// 构建出一个相对应的Topic或者Queue对象，并将这个对象加入到对象的一个map数组中
			producerExchange
	...
	/**
	 * 根据入参的类型返回一个对应的Region实现，对于这个实现是在构造方法中设置好的
	 * @Title: getRegion
	 * @Description: TODO
	 * @param destination
	 * @return
	...
	public synchronized void addBroker(Connection connection, BrokerInfo info) {
		...
		LOG.debug("{} addBroker: {} brokerInfo size: {}", new Object[] { getBrokerName(), info.getBrokerName(),
				brokerInfos.size() });
	...
	public synchronized void removeBroker(Connection connection, BrokerInfo info) {
		if (info != null) {
			...
			LOG.debug("{} removeBroker: {} brokerInfo size: {}", new Object[] { getBrokerName(), info.getBrokerName(),
					brokerInfos.size() });
	...
	public void preProcessDispatch(final MessageDispatch messageDispatch) {
		...
		if (message != null) {
			...
			if (getBrokerService().isEnableStatistics()) {
				...
				((Destination) message.getRegionDestination()).getDestinationStatistics().getProcessTime()
						.addTime(totalTime);
			}
			if (((BaseDestination) message.getRegionDestination()).isPersistJMSRedelivered()
				...
				try {
					...
					RuntimeException runtimeException = new RuntimeException("Failed to persist JMSRedeliveryFlag on "
							+ message.getMessageId() + " in " + message.getDestination(), error);
	...
	public void messageExpired(ConnectionContext context, MessageReference node, Subscription subscription) {
		...
		getRoot().sendToDeadLetterQueue(context, node, subscription,
				new Throwable("Message Expired. Expiration:" + node.getExpiration()));
	}
	@Override
		...
		try {
			if (node != null) {
				...
				if (message != null && node.getRegionDestination() != null) {
					DeadLetterStrategy deadLetterStrategy = ((Destination) node.getRegionDestination())
					...
					if (deadLetterStrategy != null) {
						if (deadLetterStrategy.isSendToDeadLetterQueue(message)) {
							...
							ActiveMQDestination deadLetterDestination = deadLetterStrategy.getDeadLetterQueueFor(
							...
							addDestination(adminContext, deadLetterDestination, false).getActiveMQDestination()
									.setDLQ();
							BrokerSupport.resendNoCopy(adminContext, message, deadLetterDestination);
							return true;
						}
					} else {
						LOG.debug("Dead Letter message with no DLQ strategy in place, message id: {}, destination: {}",
