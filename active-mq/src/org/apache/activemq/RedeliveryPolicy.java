...
/**
 * 消息重发策略
 * @ClassName: RedeliveryPolicy
 * @Description: TODO
 * @author: 孔新
 * @date: 2016年7月17日 下午4:02:27
 */
public class RedeliveryPolicy extends DestinationMapEntry implements Cloneable, Serializable {
	...
	/**
	 * 重发消息时，下面的方法返回重发消息的延迟时间，下面这个方法的具体操作流程，可以看有道云上的笔记
	 * @Title: getNextRedeliveryDelay
	 * @Description: TODO
	 * @param previousDelay为前一次延迟重发的时间
	 * @return
	 * @return: long
	 */
	public long getNextRedeliveryDelay(long previousDelay) {
		...
		if (previousDelay > 0 && useExponentialBackOff && backOffMultiplier > 1) {
			...
			if (maximumRedeliveryDelay != -1 && nextDelay > maximumRedeliveryDelay) {
				// in case the user made max redelivery delay less than redelivery delay for some
				// reason.
		...
		if (useCollisionAvoidance) {
			/*
			 * First random determines +/-, second random determines how far to go in that
			 * direction. -cgs
			 */
	...
	/**
	 * 返回一个新建的Random对象
	 * @Title: getRandomNumberGenerator
	 * @Description: TODO
	 * @return
	 * @return: Random
	 */
