...
public class ZdalConfigurationLoader {
	...
	public static volatile ClassLoader classLoader = ZdalConfigurationLoader.class
			.getClassLoader();
	 ...
	 * @return
	 */
	public synchronized ZdalConfig getZdalConfiguration(String appName,
			String dbMode, String appDsName, String configPath) {
		Map<String, ZdalConfig> configs = appDsConfigs.get(appName);
		if (configs == null || configs.isEmpty()) {
			Map<String, ZdalConfig> maps = getZdalConfigurationFromLocal(
					appName, dbMode, appDsName, configPath);
	 ...
	 * @return
	 */
	private synchronized Map<String, ZdalConfig> getZdalConfigurationFromLocal(
			String appName, String dbMode, String appDsName, String configPath) {
		...
		if (configurationFile.exists() && configurationFile.isFile()) {
			zdalConfigurationFilePathList.add("file:"
		...
		if (zdalConfigurationFilePathList.isEmpty()) {
			throw new ZdalConfigException(
					"ERROR ## There is no local Zdal configuration files for "
							+ appName + " to initialize ZdalDataSource.");
		}
		return loadZdalConfigurationContext(
				zdalConfigurationFilePathList.toArray(new String[zdalConfigurationFilePathList
						.size()]), appName, dbMode);
	}
	 ...
	 * @param idcName
	 */
	private synchronized Map<String, ZdalConfig> loadZdalConfigurationContext(
			String[] fileNames, String appName, String dbMode) {
		Map<String, ZdalConfig> zdalConfigMap = new HashMap<String, ZdalConfig>();
		try {
			FileSystemXmlApplicationContext ctx = new FileSystemXmlApplicationContext();
			ctx.setClassLoader(classLoader);
			ctx.setConfigLocations(fileNames);
			ctx.refresh();
			if (null == ctx.getBean(appName)) {
				throw new ZdalConfigException(
						"ERROR ## It must has at least one app bean in "
			...
			if (CollectionUtils.isEmpty(appBean.getAppDataSourceList())) {
				throw new ZdalConfigException(
						"ERROR ## It must has at least one appDataSource bean in "
								+ appName + " ZdalAppBean");
			}
			if (!appName.equals(appBean.getAppName())) {
				throw new ZdalConfigException(
						"ERROR ## the configured appName is "
								+ appBean.getAppName()
								+ " are not match with requested appName:"
								+ appName);
			}
			if (!dbMode.equals(appBean.getDbmode())) {
				throw new ZdalConfigException(
						"ERROR ## The configured dbMode is "
								+ appBean.getDbmode()
								+ " are not match with requested dbMode: "
								+ dbMode);
			}

			for (AppDataSourceBean appDataSourceBean : appBean
					.getAppDataSourceList()) {
	 ...
	 * @return
	 */
	protected ZdalConfig populateZdalConfig(ZdalAppBean appBean,
		...
		config.setDataSourceConfigType(appDataSourceBean
				.getDataSourceConfigType());
		// Set Rule
		...
		for (PhysicalDataSourceBean physicalDataSource : appDataSourceBean
			...
			if (null != physicalDataSource.getLogicDbNameSet()
				...
				for (String logicDBName : physicalDataSource
						.getLogicDbNameSet()) {
					config.getLogicPhysicsDsNames().put(logicDBName,
