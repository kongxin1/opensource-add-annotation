 ...
 * <p>
 * All of the methods from the {@link Connection} interface simply check to see that the
 * {@link Connection} is active, and call the corresponding method on the "delegate" provided in my
 * constructor.
 * <p>
 * Extends AbandonedTrace to implement Connection tracking and logging of code which created the
 * Connection. Tracking the Connection ensures that the AbandonedObjectPool can close this
 * connection and recycle it if its pool of connections is nearing exhaustion and this connection's
 * last usage is older than the removeAbandonedTimeout.
...
public class DelegatingConnection extends AbandonedTrace implements Connection {
	...
	/**
	 * Create a wrapper for the Connection which traces the Statements created so that any unclosed
	 * Statements can be closed when this Connection is closed.
	 * @param c the {@link Connection} to delegate all calls to.
	...
	/**
	 * Returns a string representation of the metadata associated with the innnermost delegate
	 * connection.
	 * @since 1.2.2
	 */
	...
	/**
	 * If my underlying {@link Connection} is not a <tt>DelegatingConnection</tt>, returns it,
	 * otherwise recursively invokes this method on my delegate.
	 * <p>
	 * Hence this method will return the first delegate that is not a <tt>DelegatingConnection</tt>,
	 * or <tt>null</tt> when no non-<tt>DelegatingConnection</tt> delegate can be found by
	 * traversing this chain.
	 * <p>
	 * This method is useful when you may have nested <tt>DelegatingConnection</tt>s, and you want
	 * to make sure to obtain a "genuine" {@link Connection}.
	 */
	...
	public Statement createStatement(int resultSetType, int resultSetConcurrency) throws SQLException {
		checkOpen();
		try {
			return new DelegatingStatement(this, _conn.createStatement(resultSetType, resultSetConcurrency));
		} catch (SQLException e) {
			handleException(e);
			return null;
		}
	...
	public PreparedStatement prepareStatement(String sql, int resultSetType, int resultSetConcurrency)
		...
		checkOpen();
		try {
			return new DelegatingPreparedStatement(this, _conn.prepareStatement(sql, resultSetType,
	...
	public CallableStatement prepareCall(String sql, int resultSetType, int resultSetConcurrency) throws SQLException {
		checkOpen();
		try {
			return new DelegatingCallableStatement(this, _conn.prepareCall(sql, resultSetType, resultSetConcurrency));
		} catch (SQLException e) {
			handleException(e);
			return null;
		}
