 ...
 * many configuration options, most of which are defined in the parent class. All users (based on
 * username) share a single maximum number of Connections in this datasource.
 * </p>
 * <p>
 ...
 * idle connections created using the old password are destroyed and new connections are created
...
public class SharedPoolDataSource extends InstanceKeyDataSource {
	...
	private int maxWait = (int) Math.min(Integer.MAX_VALUE, GenericObjectPool.DEFAULT_MAX_WAIT);
	...
	/**
	 * The maximum number of active connections that can be allocated from this pool at the same
	 * time, or non-positive for no limit. The default is 8.
	 */
	...
	/**
	 * The maximum number of active connections that can remain idle in the pool, without extra ones
	 * being released, or negative for no limit. The default is 8.
	 */
	 ...
	 * indefinitely. Will fail immediately if value is 0. The default is -1.
	 */
	...
	private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
		try {
			...
			SharedPoolDataSource oldDS = (SharedPoolDataSource) new SharedPoolDataSourceFactory().getObjectInstance(
