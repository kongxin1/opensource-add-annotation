 ...
 * {@link javax.sql.ConnectionPoolDataSource}, but still include a {@link java.sql.DriverManager}
 * implementation. <code>ConnectionPoolDataSource</code>s are not used within general applications.
 * They are used by <code>DataSource</code> implementations that pool <code>Connection</code>s, such
 * as {@link org.apache.commons.dbcp.datasources.SharedPoolDataSource}. A J2EE container will
 * normally provide some method of initializing the <code>ConnectionPoolDataSource</code> whose
 * attributes are presented as bean getters/setters and then deploying it via JNDI. It is then
 * available as a source of physical connections to the database, when the pooling
 * <code>DataSource</code> needs to create a new physical connection.
 * </p>
 * <p>
 * Although normally used within a JNDI environment, the DriverAdapterCPDS can be instantiated and
 * initialized as any bean and then attached directly to a pooling <code>DataSource</code>.
 * <code>Jdbc2PoolDataSource</code> can use the <code>ConnectionPoolDataSource</code> with or
 * without the use of JNDI.
 * </p>
 * <p>
 * The DriverAdapterCPDS also provides <code>PreparedStatement</code> pooling which is not generally
 * available in jbdc2 <code>ConnectionPoolDataSource</code> implementation, but is addressed within
 * the jdbc3 specification. The <code>PreparedStatement</code> pool in DriverAdapterCPDS has been in
 * the dbcp package for some time, but it has not undergone extensive testing in the configuration
 * used here. It should be considered experimental and can be toggled with the
 * poolPreparedStatements attribute.
 * </p>
 * <p>
 * The <a href="package-summary.html">package documentation</a> contains an example using catalina
 * and JNDI. The <a href="../datasources/package-summary.html">datasources package documentation</a>
 * shows how to use <code>DriverAdapterCPDS</code> as a source for <code>Jdbc2PoolDataSource</code>
 * without the use of JNDI.
 * </p>
...
public class DriverAdapterCPDS implements ConnectionPoolDataSource, Referenceable, Serializable, ObjectFactory {
	...
	public PooledConnection getPooledConnection(String username, String pass) throws SQLException {
		...
		/*
		 * public GenericKeyedObjectPool(KeyedPoolableObjectFactory factory, int maxActive, byte
		...
		if (isPoolPreparedStatements()) {
			if (getMaxPreparedStatements() <= 0) {
				...
				stmtPool = new GenericKeyedObjectPool(null, getMaxActive(), GenericKeyedObjectPool.WHEN_EXHAUSTED_GROW,
						0, getMaxIdle(), false, false, getTimeBetweenEvictionRunsMillis(), getNumTestsPerEvictionRun(),
						getMinEvictableIdleTimeMillis(), false);
			} else {
				// since there is limit, create a prepared statement pool without an eviction thread
				// pool has LRU functionality so when the limit is reached, 15% of the pool is
				// cleared.
				// see org.apache.commons.pool.impl.GenericKeyedObjectPool.clearOldest method
				stmtPool = new GenericKeyedObjectPool(null, getMaxActive(), GenericKeyedObjectPool.WHEN_EXHAUSTED_GROW,
						0, getMaxIdle(), getMaxPreparedStatements(), false, false, -1, 0, 0, // -1
																								// tells
																								// the
																								// pool
																								// that
	...
	public Reference getReference() throws NamingException {
		...
		ref.add(new StringRefAddr("maxActive", String.valueOf(getMaxActive())));
	 ...
	 * </p>
	 * <p>
	 * If <code>props</code> contains "user" and/or "password" properties, the corresponding
	 ...
	 * {@link #setPassword(String)} overwrite the values of these properties if
	 * <code>connectionProperties</code> is not null.
	 * </p>
	...
	/**
	 * Get the value of description. This property is here for use by the code which will deploy
	 * this datasource. It is not used internally.
	...
	/**
	 * Set the value of description. This property is here for use by the code which will deploy
	 * this datasource. It is not used internally.
	...
	/**
	 * Set the driver classname. Setting the driver classname cause the driver to be registered with
	 * the DriverManager.
	...
	/**
	 * The maximum number of active statements that can be allocated from this pool at the same
	 * time, or non-positive for no limit.
	 */
	...
	/**
	 * The maximum number of statements that can remain idle in the pool, without extra ones being
	...
	/**
	 * Returns the number of statements to examine during each run of the idle object evictor thread
	 * (if any). *see #setNumTestsPerEvictionRun *see #setTimeBetweenEvictionRunsMillis
	 */
	public int getNumTestsPerEvictionRun() {
		return _numTestsPerEvictionRun;
	}
	/**
	 * Sets the number of statements to examine during each run of the idle object evictor thread
	 * (if any).
	 * <p>
	 * When a negative value is supplied,
	 * <tt>ceil({*link #numIdle})/abs({*link #getNumTestsPerEvictionRun})</tt> tests will be run.
	...
	/**
	 * Returns the minimum amount of time a statement may sit idle in the pool before it is eligible
	 * for eviction by the idle object evictor (if any). *see #setMinEvictableIdleTimeMillis *see
	...
	/**
	 * Sets the minimum amount of time a statement may sit idle in the pool before it is eligable
	 * for eviction by the idle object evictor (if any). When non-positive, no objects will be
