...
/**
 * <p>
 * This class has been copied from Commons Collections, version 3.1 in order to eliminate the
 * dependency of pool on collections. It has package scope to prevent its inclusion in the pool
 * public API. The class declaration below should *not* be changed to public.
 * </p>
 * A doubly-linked list implementation of the {@link List} interface, supporting a
 * {@link ListIterator} that allows concurrent modifications to the underlying list.
 * <p>
 * Implements all of the optional {@link List} operations, the stack/queue/dequeue operations
...
class CursorableLinkedList implements List, Serializable {
	 ...
	 * that they are returned by the specified {@link Collection}'s {@link Iterator}. The behavior
	 * of this operation is unspecified if the specified collection is modified while the operation
	 * is in progress. (Note that this will occur if the specified collection is this list, and it's
	 * nonempty.)
	 ...
	 * order that they are returned by the specified {@link Collection}'s {@link Iterator}. The
	 * behavior of this operation is unspecified if the specified collection is modified while the
	 ...
	 *             prevents it from being added to this list.
	 * @throws IllegalArgumentException if some aspect of one of elements of the specified
	...
	/**
	 * Inserts the specified element at the beginning of this list. (Equivalent to
	 * {@link #add(int,java.lang.Object) <tt>add(0,o)</tt>}).
	...
	/**
	 * Inserts the specified element at the end of this list. (Equivalent to
	 * {@link #add(java.lang.Object)}).
	 * @param o element to be appended to this list.
	 * @return <tt>true</tt>
	 */
	...
	public void clear() {
		/*
		 * // this is the quick way, but would force us // to break all the cursors _modCount++;
		 * _head.setNext(null); _head.setPrev(null); _size = 0;
		 */
	...
	/**
	 * Returns a {@link ListIterator} for iterating through the elements of this list. Unlike
	 * {@link #iterator}, a cursor is not bothered by concurrent modifications to the underlying
	 * list.
	 * <p>
	 * Specifically, when elements are added to the list before or after the cursor, the cursor
	 * simply picks them up automatically. When the "current" (i.e., last returned by
	 * {@link ListIterator#next} or {@link ListIterator#previous}) element of the list is removed,
	 * the cursor automatically adjusts to the change (invalidating the last returned value--i.e.,
	 * it cannot be removed).
	 * <p>
	 * Note that the returned {@link ListIterator} does not support the
	 * {@link ListIterator#nextIndex} and {@link ListIterator#previousIndex} methods (they throw
	 * {@link UnsupportedOperationException} when invoked.
	 * <p>
	 * Historical Note: In previous versions of this class, the object returned from this method was
	 * required to be explicitly closed. This is no longer necessary.
	 * @see #cursor(int)
	 * @see #listIterator()
	 ...
	 * such that {@link ListIterator#next} will return the element at the specified index (if any)
	 * and {@link ListIterator#previous} will return the element immediately preceding it (if any).
	 * Unlike {@link #iterator}, a cursor is not bothered by concurrent modifications to the
	 * underlying list.
	 * @see #cursor()
	...
	/**
	 * Compares the specified object with this list for equality. Returns <tt>true</tt> if and only
	 * if the specified object is also a list, both lists have the same size, and all corresponding
	 * pairs of elements in the two lists are <i>equal</i>. (Two elements <tt>e1</tt> and
	 * <tt>e2</tt> are <i>equal</i> if <tt>(e1==null ? e2==null :
	 * e1.equals(e2))</tt>.) In other words, two lists are defined to be equal if they contain the
	 * same elements in the same order. This definition ensures that the equals method works
	 ...
	 * this list does not contain this element. More formally, returns the lowest index <tt>i</tt>
	 * such that <tt>(o==null ? get(i)==null : o.equals(get(i)))</tt>, or -1 if there is no such
	 * index.
	...
	public int indexOf(Object o) {
		...
		if (null == o) {
			for (Listable elt = _head.next(), past = null; null != elt && past != _head.prev(); elt = (past = elt)
					.next()) {
				if (null == elt.value()) {
					return ndx;
				}
	...
	public int lastIndexOf(Object o) {
		...
		// single time through the loop.
		if (null == o) {
			for (Listable elt = _head.prev(), past = null; null != elt && past != _head.next(); elt = (past = elt)
					.prev()) {
				if (null == elt.value()) {
					return ndx;
				}
	 ...
	 * index i such that <tt>(o==null ? get(i)==null : o.equals(get(i)))</tt> (if such an element
	 * exists).
	 ...
	 * other words, removes from this list all the elements that are not contained in the specified
	 * collection.
	 ...
	 * @exception ArrayStoreException if the runtime type of the specified array is not a supertype
	...
	public Object[] toArray(Object a[]) {
		...
		if (a.length > _size) {
			a[_size] = null; // should we null out the rest of the array also? java.util.LinkedList
								// doesn't
		}
		return a;
	}
	...
	// --- protected methods ------------------------------------------
	/**
	 * Inserts a new <i>value</i> into my list, after the specified <i>before</i> element, and
	 * before the specified <i>after</i> element
	...
	/**
	 * Removes the given {@link org.apache.commons.collections.CursorableLinkedList.Listable} from
	 * my list.
	 */
	/**
	 * 将入参从本列表中删除，并且从所有的光标中删除
	 * @Title: removeListable
	 * @Description: TODO
	 * @param elt
	 * @return: void
	 */
	...
	/**
	 * Returns the {@link org.apache.commons.collections.CursorableLinkedList.Listable} at the
	 * specified index.
	...
	/**
	 * Removes a {@link CursorableLinkedList.Cursor} from the set of cursors to be notified of
	...
	/**
	 * Informs all of my registered cursors that the specified element was just removed from my
	 * list.
	 */
	/**
	 * 将入参从所有的光标中删除
	 * @Title: broadcastListableRemoved
	 * @Description: TODO
	 * @param elt
	 * @return: void
	 */
	 ...
	 * A sentry node.
	 * <p>
	 * <tt>_head.next()</tt> points to the first element in the list, <tt>_head.prev()</tt> to the
	 ...
	 * <tt>== _head.next()</tt> and <tt>== _head.prev()</tt> to determine if a given
	 * {@link org.apache.commons.collections.CursorableLinkedList.Listable} is the first or last
	 * element in the list.
	 */
	/**
	 * 链表的空节点
...
class CursorableSubList extends CursorableLinkedList implements List {
	...
	/**
	 * Test to see if my underlying list has been modified by some other process. If it has, throws
	 * a {@link ConcurrentModificationException}, otherwise quietly returns.
